# =============================================================================
# Staged code (qf-6z9.4): OFT-vs-Trotter error utilities.
#
# These were the cross-domain consistency drivers used during the early
# Trotter cache work. They exercise `time_oft!` / `trotter_oft!` (now in
# `src/staging/ofts.jl`) at sampled energy labels and report the per-omega
# operator-norm difference between exact and Trotter time evolution.
#
# Equivalent diagnostics now live in the regression test suite
# (`test/test_dm_scaling.jl::DMTST-06b`, NUFFT-vs-analytical paths) and the
# qf-7xt convergence scratches (`scripts/scratch_*_ref_convergence.jl`).
# Reattaching this file requires un-staging `src/staging/ofts.jl` first.
# =============================================================================
