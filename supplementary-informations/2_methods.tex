\part{Theory of Quantum Gibbs Sampling}

The Metropolis-Hastings algorithm in \hyperref[parag:MH]{Sec.~\ref*{parag:MH}} became ubiquitous for any kind of sampling problems in the classical world, so much so that it became one of the most important algorithms of the 20th century \cite{dongarra2000guest}. While we have no doubt the classical Metropolis algorithm will continue to advance and have even more successes, it naturally has scaling limitations, just as most algorithms do. The dimensional explosion of the state space in quantum systems and similarly in large classical systems can pose a problem for even the state-of-the-art implementations of the algorithm [REF] \todo{ref}. Now that quantum computers have been on the horizon for some years, it could be beneficial to explore its capabilities for (Gibbs) sampling. Then one of the natural questions to ask is,
$$\textit{Is it possible to generalize the Metropolis algorithm to the quantum setting?}$$
The answer has only been recently given in a foundational work \cite{temme2011quantum}, that established the notion of the \textit{quantum Metropolis algorithm}, and with that created a lot of excitement for the field.
...
One way to get rid of the rewinds, is to work with a continuous-time Markov sampling instead of a discrete one. In this case, we never have to worry about a subpar jump and its rewind, but rather worry about the rate of the jumps. As we will see in this chapter, we are constrained to use low rates for the jumps, or else the approximations on the way would get out of hand. Nevertheless, this might be an improvement to the Metropolis algorithm, since this weak disturbance of the system is also a key constraint for the rewind, not only of the continuous time generator picture.

Concerning the question of detailed balance, it must be stated that it is not a priori clear how and if at all $exact$ detailed balance is necessary for Gibbs sampling \textit{in practice}. Why we prefer having detailed balance has more of a mathematical reasoning: it allows us to work with well established techniques for reversible Markov semigroups; it gives guarantees that the sampling eventually reaches the Gibbs state and thus helps with rigorous mixing time bound proofs. However, implementing an exact detailed balanced algorithm on a computer could easily break that nice, yet fragile symmetry. Even the algorithm we will introduce here, will break its proven detailed balance the moment we put it in quantum circuit form. The hope is that once there is an exact detailed balance framework established, it can be used as a blueprint for algorithms that break the symmetry in a controlled and rigorous way. 

The goal for this part of the thesis is to (i) introduce a recently proposed approximate and exact detailed balanced Lindbladians and their implementations from Chen et al. \cite{chen2023quantum,chen2023efficient}; (ii) Analyse it and extend its theory with a more implementation focused mindset, e.g. proposing transition rates that are provably close to optimal, while smoother than the Metropolis function. Work out rigorously the quadrature, Trotter, and generator-splitting errors that appear in actual implementations; (iii) Contrast this framework, with a newer iteration from \cite{ding2025efficient}, that though opens up opportunities, and new design choices, it cannot clearly beat the Chen et al. framework. 

\chapter{CKBG Framework}
\smallskip
\label{sec:approx-gns-lindbladian}
\section{Approximate GNS Lindbladian} While the Davies generator, introduced in \hyperref[subsec:davies]{Sec.~\ref*{subsec:davies}}, satisfies GNS detailed balance, direct implementation of it on a computer remains infeasible. The main obstacle is that it requires perfect energy estimates for the jumps that can resolve each of its Bohr frequency component in the generator \eqref{eq:davies-generator}.
For small systems this might even be possible, and we could distinguish each and every Bohr frequency component, then weigh each of them with the prescribed transition rates (that satisfy KMS condition). But as we increase the system size and consider more interesting systems, the Bohr frequencies get exponentially close to each other. This poses an inherent metrology problem for implementations, because of the energy-time uncertainty principle. Which means that the more precise an energy value needs to be estimated, the longer Hamiltonian time evolution is necessary to do in the quantum computer (e.g. with QPE), thus leading to exponential costs. As we see, the crux in quantum Gibbs sampling is then not only to have a detailed balanced construction but also to have an efficiently implementable one.

A natural question to ask is, what happens with detailed balance if we allow a controlled error around the Bohr frequencies that we need to estimate. As it turns out from \cite{chen2023quantum}, these smoothened Davies generators still \textit{approximately} satisfy detailed balance. To see why, it is constructive to start from the GNS detailed balanced Davies generator \eqref{eq:davies-gns-db}. There, it was useful to work with the Heisenberg evolution of the applied jumps:
$$
...
$$
As a reminder: in quantum computers we always have to work in the time domain, and the Fourier transform is the tool that brings us to the energy domain. In order to implement the Davies generator, one would need a Fourier transform that maps exactly to $A_\nu$, i.e. an OFT with $f(t) = 1$, which after the Fourier transform becomes $\hat{f}(\omega) = \sqrt{2\pi} \delta(\omega)$:
\begin{equation}
    \hat{A}_{f=1}(\omega) = \sqrt{2\pi}\sum_{\nu \in B_H} A_\nu \delta(\omega - \nu).
\end{equation}
An exact mapping would require a computation over the full, infinitely large time domain. For a more feasible computation, the idea is to choose an $f(t)$ such that it has some decaying tail that effectively truncates the Fourier transforms to some more manageable subdomains. From \eqref{eq:oft} we can see the effect of a non-trivial filter function: the OFT leads to $\hat{A}(\omega)$ instead of $A_\nu$, which includes neighbouring energy contributions centred around $A_\nu$. A priori it is not clear how these controllable errors for the jump operators affect the detailed balanced structure of the generator. However, working it through, the authos in \cite{chen2023quantum} prove, that it leads to an approximately GNS detailed balance Lindbladian. 

There is some freedom in how we can choose the filter function, but it is necessary for it to be a well-behaving $L^1$ integrable distribution. Clearly, for a not too costly implementation, $f(t)$ has to decay rapidly in the time domain as $|t|\rightarrow \infty$, and concurrently its Fourier integral needs to be efficiently amenable to quadrature techniques. Moreover, its Fourier transform $\hat{f}(\omega)$ has to be smooth too.
...
\end{equation}
The width of the Gaussian $1 / (2 \sigma)$ is a tuneable parameter, that determines how strongly we smoothen the jumps, thus the generator. It is directly affecting the cost of the quantum algorithm. Gaussians have the nice property, that their Fourier transforms are also Gaussians, albeit with an inverse width $2\sigma$,
...
\end{equation}
As seen in \hyperref[fig:oft-smearing]{Fig.~\ref*{fig:oft-smearing}}, the smooth jump operators that the Fourier transforms are mapping to, can be understood as Gaussian distributions centred around each Bohr frequency $\nu$ with an uncertainty of $2\sigma$,
\begin{equation}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.85\textwidth]{plots/oft_smearing.pdf}
    \caption[OFT Gaussian smearing around two Bohr frequencies.]{%
        The Operator Fourier Transform (OFT) replaces each exact $\nu$ contribution of a jump operator $A$ by a Gaussian distribution
        $\hat{f}(\omega - \nu) = (2\pi\sigma^2)^{-1/4}\exp(-\omega^2/(4\sigma^2))$.
        Shown at $\beta^\sharp = 10$, $\sigma = 1/\beta^\sharp = 0.1$ around two adjacent Bohr frequencies $\nu_1 = -0.1$, $\nu_2 = 0.2$. The shaded overlap region marks the energies for which the OFT cannot cleanly resolve $\nu_1$ from $\nu_2$.}
    \label{fig:oft-smearing}
\end{figure}
\todo{Fix title to algorithmic beta sharp and width too}
A subtlety we need to explain here, so the reader is not confused by strange notations: by $\beta$ we denote the \emph{physical} inverse temperature, and by $\beta^\sharp = 2 \|H\| \beta$ the \emph{algorithmic} inverse temperature. The distinction arises because in the algorithm, due to the Quantum Fourier Transform, we have to rescale the spectrum to $\mathrm{Spec}(H) = [0, 0.5 - \varepsilon]$ for some small $\varepsilon > 0$ to avoid aliasing. The moment the spectrum is rescaled however, $\beta$ has to be upscaled to $\beta^\sharp$ to keep correct physics. Hence, all numerical simulations are first scaled to algorithmic temperature, energy, time units and then converted back to physical ones. But while the algorithm runs in $\beta^\sharp$ we will suppress this notation for simplicity whenever we can.

The Gaussian filter \eqref{eq:gaussian-filter-t} is not the only function that could be used. Gevrey functions also fulfil the necessary requirements and have been successfully used for KMS detailed balanced Lindbladians in \cite{ding2025efficient}, also see \hyperref[sec:dll]{Sec.~\ref*{sec:dll}}. Until then let us keep ourselves to the Gaussians. 

We will call the Davies generator with the smoothened jump operators \eqref{eq:smoothed-jump} as \textit{Chen-Kastoryano-Brandão-Gilyén} or \textit{CKBG Lindbladian} after the authors, and it takes the following form in the Schrödinger picture:
\begin{equation}
    \label{eq:L-ckbg}
...
The sum runs over the set of jumps $\{A^a\}_{a\in\mathcal{A}}$, that can be chosen in any way that ensures ergodicity: usually a varied enough set that can break all symmetries that the Hilbert space might have, and where each jump also has its adjoint included,
\begin{equation}
...
\end{equation}
Here, the norm will be used to normalize the jumps and its purpose is (i) to help the block encodings keep a smaller subnormalization factor in implementation, and (ii) to allow the mixing analysis decouple the structure from the Lindbladian's strength or rate. More jumps necessarily lead to higher norms and jump rate, but we are only interested in the structure of the generator's mixing. 

The naive set of single-site Pauli jumps work already quite well, and since they are unitary, some block encodings ancillas and gates in the circuits can be avoided. (In this case, the normalization factor in \eqref{eq:ergodic-set-jumps} becomes the total number of jumps $|\mathcal{A}|$.) Even if single-site jumps are used as the core, the Lindblad jumps of the evolution, $\{L_j\}_{j\in\mathcal{J}} = \{\sqrt{\gamma(\omega)}\hat{A}_a(\omega)\}$ are not restricted to that one single site. We can see this from the OFT construction \eqref{eq:oft}: as the Pauli operators evolve over time in the Heisenberg picture, its support spreads out spatially. We can estimate this size by the Lieb-Robinson bound to see that for the small system sizes that we can numerically simulate, these jumps spread out over the whole lattice. See the Appendix for more details [APPENDIX]. \todo{appendix LR bound}
...
\end{equation}
A suitable function can be the following tuneable Gaussian,
...
\todo{caption fix, bar beta fix}
\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{plots/transition_weights.pdf}
    \caption[Transition weights $\gamma(\omega)$ for the CKG variants.]{%
        Transition weights $\gamma(\omega)$ for the four CKG variants used in
        the thesis, drawn on the rescaled-$\omega$ axis with
        $\sigma = 1/\beta^\sharp$ and $\beta^\sharp = 10$. \emph{Left:} the
        Gaussian weight $\gamma_G(\omega) = \exp(-(\omega + \omega_\gamma)^2 /
        (2\sigma_\gamma^2))$ centred at $\omega_\gamma = -1/\beta^\sharp$.
        \emph{Right:} the kinky Metropolis weight $\gamma_M$ \eqref{eq:shifted-metro-func},
        its smooth-Metropolis lift $\gamma_M^{(s)}$ at $s = 0.25$
        \eqref{eq:smooth-metro}, and at $s=2$ which is also called as the Glauber weight $\gamma_{\mathrm{Gl}}$}
    \label{fig:transition-weights}
\end{figure}

In \hyperref[fig:transition-weights]{Fig.~\ref*{fig:transition-weights}} some later used \emph{shifted} transition weights are plotted, but we can already see the qualitative differences between them. For our current CKBG setting (without the shift, i.e. the dashed vertical line is at $\omega = 0$) you get transition rates that fulfil the KMS condition \eqref{eq:KMS-condition-2}. The Gaussian rate on the left is much more restrictive compared to the Metropolis-like transitions that we will consider later on. Nevertheless, we can already work out what happens to  detailed balance, if any, when we have with these Gaussian smoothed jump operators \eqref{eq:oft}. 

In its current form \eqref{eq:L-ckbg} has been shown to satisfy \textit{approximate GNS detailed balance} in \cite{chen2023quantum}. Quantitatively, we have understood the severity of breaking detailed balance as the norm of the anti-Hermitian part $\|\mathcal{A}(\rho, \mathcal{L})\|_{2\rightarrow 2}$ of the generator's discriminant $\mathcal{D}(\rho, \mathcal{L})$, see \hyperref[def:approx-db]{Definition \ref*{def:approx-db}}. Thus, if we can choose algorithm parameters such that the norm of the anti-Hermitian part is below some $\varepsilon$, we can guarantee that the fixed point $\rho_\text{fix}(\mathcal{L})$ is close to the target state $\rho_\beta$.
...
\end{corollary}
\noindent Chen et al. provides the proof, which is indeed less than obvious. The core thought is that perturbations to the parent Hamiltonian $\mathcal{H}$ with $\mathcal{A}$, leads to only small changes for the eigenvectors with separated eigenvalues. In fact, one can show that the top eigenvalue of $\mathcal{H}$ will not deviate further from $0$ (its detailed balanced value) than $$|\lambda_1(\mathcal{H})| \leq \|\mathcal{A}\|_{2\rightarrow 2}.$$ We can combine all of this with the CKBG Lindbladian and find that it is approximately GNS detailed balanced.
\begin{theorem}
...
One of the main culprits leading to deviations from the Gibbs state is the presence of non-secular terms $\nu_1 \neq \nu_2$ in the CKBG Lindbladian \eqref{eq:L-ckbg}. For the \hyperref[subsec:davies]{Davies generator} we had to apply the secular approximation in order to get a correct, physical generator. But here, with the smoothened jump operators \eqref{eq:oft} we have reintroduced some off-diagonal entries in the Kossakowski matrix (while preserving physicality). This becomes clear when we derive its analytical form by computing the integral of the over the energies in \eqref{eq:L-ckbg} for the transition part in the Lindbladian:
...
\end{equation}
In other words, Gibbs sampling with the $\mathcal{L}_\text{CKBG}$ from \eqref{eq:L-ckbg} and with $\sigma > 0$, effectively leads to some higher temperature Gibbs state. The error structure is more complicated however than just raised temperatures. The non-secular terms are also a source of errors, and an even less clear one at that. By taking the $\sigma \rightarrow 0$ limit, all errors to the GNS detailed balance vanish: $\beta_\text{eff} \rightarrow \beta$ and as the Gaussian filters become delta peaks in this limit, they also eliminate any off-diagonal elements and only $\nu_1 = \nu_2$ remains.

The CKBG framework already provided a step forward in Gibbs sampling, it is provably approximate detailed balanced and avoids previous costly obstacles of the quantum Metropolis algorithm. However, it still suffers from the core metrological issue: in order to prepare a Gibbs state with high precision, it needs close to perfect energy estimations on the quantum computer (by $\sigma \to 0$). Nevertheless, it proved to be the right scaffolding for an actual breakthrough in quantum Gibbs sampling that we analyse in the next chapter.

\chapter{CKG Framework}
\smallskip
\label{sec:kms-lindbladian}
\section{KMS Lindbladian} While the previous approximate GNS detailed balanced Lindbladian was a result on its own, its improvement is what got researchers (with myself included) thrilled about quantum Gibbs sampling. In \cite{chen2023efficient}, Chen et al. have managed to come up with a structure for the Lindbladian where they do not fight the above-mentioned errors, but rather lean into them and fix them all at once with an aptly engineered coherent term. The result is an exact KMS detailed balanced Lindbladian instead of one that tries to approximate the GNS detailed balance Davies generator using a lot of quantum resources. In order to better see how terms in the Lindbladian interact with each other to get KMS detailed balance, it is constructive to write the Lindbladian in this general form:
\begin{equation}
...
\end{alignat}
For $\mathcal{R}^\dagger$ we implicitly took the Kossakowski matrix to be positive semidefinite and thus Hermitian, $\alpha_{\nu_1, \nu_2} = \bar\alpha_{\nu_2, \nu_1}$. Although this needs to be checked for every new constructions, without it, we would not have a GKSL Lindbladian.

Other than the additional coherent term $\mathcal{B}$, the rest of the setup is unchanged: we take the same Gaussian filter functions for the jump operators \eqref{eq:oft}; we keep the Gaussian transition weights $\gamma(\omega)$ \eqref{eq:gaussian-transition}; Consequently, we have the exact same Kossakowski matrix $\alpha_{\nu_1, \nu_2}$ too \eqref{eq:kossakowski-gaussian} with one small but pivotal difference, the temperature. We set the temperature such that the diagonal entries of $\alpha_{\nu_1,\nu_2}$ satisfy the KMS condition, i.e. as $\beta := \beta_\text{eff}$. This instantly makes $\gamma(\omega)$ itself break the original KMS condition, however, the goal here is to satisfy the condition \textit{with} the Gaussian filters together. From the relation \eqref{eq:beta-eff}, it is clear that the Gaussian parameters and the temperature are all intertwined. In the KMS DB case, there is a convenient way to choose them to keep the Kossakowski matrix necessarily skew-symmetric, by 
\begin{equation}
    \sigma = \omega_\gamma = \sigma_\gamma = \frac{1}{\beta}.
\end{equation}
Nevertheless, we will keep the following expressions general, because it is not a priori clear how best to choose these parameters for a faster Gibbs state preparation. This is one of the questions we have tackled in our numerical analysis, see \todo{ref to results: mixing time dependence on more or less off-diagonal terms.}

Previously the off-diagonal terms $\nu_1 \neq \nu_2$ were either eliminated via secular approximations (Davies) or were one of the reasons for deviation from GNS detailed balance (CKBG Lindbladian). Now these terms are part of the design. But that also means there must be a condition on how these off-diagonal terms relate to each other in order to attain KMS detailed balanced, similar to the diagonal entries in \eqref{eq:kossakowski-diag-condition}.
Different parts of the Lindbladian \eqref{eq:ckg-L-parts} behave differently when scrutinized over KMS DB. In fact, while the transition part $\mathcal{T}$ keeps its structure under Gibbs conjugations, the decay $\mathcal{R}$ and coherent $\mathcal{B}$ parts mix together.

Let us start with the transition part: for it to satisfy the KMS DB condition, it needs to be self-adjoint with respect to the KMS inner product, or equivalently: 
\begin{equation} \label{eq:transition-self-adjoint}
    \mathcal{T}^\dagger = \Gamma_\rho^{-1}\, \circ \, \mathcal{T}\, \circ\, \Gamma_\rho^{1},
\end{equation}
with $\Gamma_\rho(\cdot) := \rho^{\frac{1}{2}} (\cdot) \rho^{\frac{1}{2}}$, where $\rho$ is the fixed point of the quantum Markov semigroup, i.e. in our case $\rho = \rho_\beta$. The equality indeed holds if each jump $A_a$ has its Hermitian conjugate in the jumps set $\mathcal{A}$, and most importantly if the Kossakowski matrix has a specific skew-symmetry:
...
Now take the remaining parts of the Lindbladian, $\mathcal{R}$ and $\mathcal{B}$. If $\mathcal{R}$ is conjugated with the Gibbs state, we find the following terms: 
\begin{equation} \label{eq:decay-gibbs-conjugation}
    \Gamma_\rho^{-1}\, \circ \, \mathcal{R}\, \circ\, \Gamma_\rho^{1}(\cdot) =\sum_{\nu_1, \nu_2} \left( e^{\beta(\nu_1 - \nu_2)/2}R_{\nu_1, \nu_2}(\cdot) + (\cdot) R_{\nu_1, \nu_2} e^{-\beta(\nu_1 - \nu_2)/2}\right),
\end{equation}
with $R_{\nu_1, \nu_2} := \sum_a \alpha_{\nu_1, \nu_2} (A^a_{\nu_2})^\dagger A^a_{\nu_1}$. It is evident, that the two terms pick up a different factor in the anti-commutator of $\mathcal{R}$ and thus no simple condition on $\alpha_{\nu_1, \nu_2}$ (like the skew-symmetry), can lead to KMS detailed balance alone. However, the key point to notice is that the coherent term $\mathcal{B}$ transforms similarly as the decay part under the KMS conjugation, 
\begin{equation} \label{eq:coherent-gibbs-conjugation}
    \Gamma_\rho^{-1}\circ\mathcal{B}\circ\Gamma_\rho(\cdot) = \sum_\nu \left(e^{+\beta\nu/2}\,B_\nu\, (\cdot) \;-\; (\cdot)\,B_\nu e^{-\beta\nu/2}\right),
\end{equation}
with the usual Bohr frequency decomposition $B = \sum_\nu B_\nu$. Then, one can dream up a form for $B$ such that all the detailed balance violating terms in the decay part in \eqref{eq:decay-gibbs-conjugation} cancel (while the skew-symmetry \eqref{eq:skew-symmetry} for the Kossakowski matrix holds). This is exactly what Chen et al. figured out in \cite{chen2023efficient}.

Then, for KMS DB the following equality has to hold:
\begin{equation}
    \left(-\frac{1}{2}\mathcal{R} + \ii \mathcal{B}\right) = \Gamma_\rho^{-1}\circ\left(-\frac{1}{2}\mathcal{R} - \ii \mathcal{B}\right)\circ\Gamma_\rho.
\end{equation}
With the Hilbert-Schmidt adjoint on the LHS and the Gibbs conjugation on the RHS. In other words, we are looking for an expression of $\mathcal{B}$ such that $\mathcal{R}$ and $\mathcal{B}$ together are self-adjoint with respect to the KMS inner product. If we write all these terms out using \eqref{eq:decay-gibbs-conjugation} and \eqref{eq:coherent-gibbs-conjugation}, and identify $\nu = \nu_1 - \nu_2$, then we can match the prefactors of the same type of terms on both sides. The left-multiplication prefactors give: 
\begin{equation}
    -\frac{1}{2}R_{\nu} + \ii B_\nu = -\frac{1}{2} \ee^{\beta\nu / 2}R_{\nu} - \ii \ee^{\beta\nu / 2} B_\nu
\end{equation}
which rearranges to
...
\smallskip
\label{sec:ckg-transition-weights}
\section{Designing transition weights}
Only allowing transitions with energies concentrated as a Gaussian around some $\omega_\gamma$ can be very restricting, see \hyperref[fig:transition-weights]{Fig.~\ref*{fig:transition-weights}} for a quite warm $\beta^\sharp$ value, which inevitably leads to slower thermalization. The question is, \textit{Are there more accepting rates $\gamma(\omega)$ that also preserve KMS detailed balance of the CKG Lindbladians?} Yes there are, and we can find them by finding a clear connection between the condition on the transition weights $\gamma(\omega)$ and the necessary skew-symmetry of the Kossakowski matrix $\alpha_{\nu_1, \nu_2}$.
\begin{lemma} \label{lem:shifted-kms-condition}
    ...
    $$
    is skew-symmetric by \eqref{eq:skew-symmetry}. Here, $\hat{f}$ is any filter with $|\hat{f}(\omega)|^2 = |\hat{f}(-\omega)|^2$, like the Gaussian filter from \eqref{eq:gaussian-filter-w}.
\end{lemma}
...
Hence, we can take any transition weight function $\tilde{\gamma}(\omega)$ that satisfies the KMS condition, shift it to $\gamma(\omega)$ and find a KMS detailed balanced CKG Lindbladian. This is quite interesting, since we can import canonically well-working transition weights that have been used for Gibbs sampling in the past. One such function, is the \textit{Metropolis function}, which takes the following form after the prescribed shift from \hyperref[lem:shifted-kms-condition]{Lemma~\ref*{lem:shifted-kms-condition}}: 
...
From \hyperref[fig:transition-weights]{Fig.~\ref*{fig:transition-weights}} it becomes apparent why the Metropolis function leads to better mixing compared to the previous Gaussian case. It accepts almost all transitions that lower the energy with probability 1, and only suppresses energy increasing jumps. Since the Metropolis function is the centrepiece of the Metropolis-Hastings algorithm \cite{metropolis1953equation, hastings1970monte} which is already decades old, the community had time to rigorously analyse why it works so well. 
Peskun showed in \cite{peskun1973optimum} that among all reversible chains with the same proposal and target, the one with the pointwise maximal off-diagonal transition rates (between states $i \neq j$), has the smallest asymptotic variance for any estimator. The (unshifted) Metropolis acceptance is the unique pointwise maximum satisfying detailed balance with $\|\gamma\|_\infty\leq 1$. Tierney extends these results in \cite{tierney1998note} by finding that the spectral gap among these reversible chains, is also the largest for the Metropolis transition, hence it is the fastest mixing among them.

These are strong results, but they apply only for classical Markov chains and as it turns out, they do not generally hold in the quantum case. It is an open question if there exists a quantum variant of the above results. At the moment it is not clear if and how the off-diagonal elements of a Kossakowski matrix $\alpha_{\nu_1, \nu_2}$ improve upon the mixing times of quantum Markov semigroups. There are recent numerical evidences in the general KMS detailed balanced framework \cite{chen2024boosting,hahn2025efficient} suggesting that there is not one choice that fits all problems. But the lack of analytical results is apparent.

Even if for the general Kossakowski matrix in the quantum case we do not see how to prove Peskun-like statement, within the CKG framework with given Gaussian filter functions \eqref{eq:gaussian-filter-w}, we can prove that the Metropolis transition rate $\gamma_M$ leads to the fastest mixing.
...
\begin{proof}
    ...
    $\textup{Gap}\left(\mathcal{L}^\dagger\right) = \inf_{X} \mathcal{E}(X, X) / \|X\|^2_\textup{KMS}$ with $X\neq 0$ and $\tr(\rho_\beta X) = 0$, and with the Dirichlet form $\mathcal{E}^\gamma(X, X) := -\langle X, (\mathcal{L}^\gamma)^\dagger(X) \rangle_\textup{KMS}$. See also \hyperref[sec:prelim-qlsi]{Sec.~\ref*{sec:prelim-qlsi}}, and note that the Dirichlet form is defined in the Heisenberg picture, hence the adjoint $\mathcal{L}^\dagger$. Consequently, the proof boils down to showing that $$\mathcal{E}^\Delta(X, X) \geq 0 \quad\forall X \in \mathcal{B(H)},$$
...
There is an interesting technique that is worth mentioning here, as it can help us design transition functions and Kossakowski matrices.
...
By choosing $g$ as the inverse $\ell_1$ weight of the Gaussians, $g(x) = \frac{1}{\sqrt{2\pi}\sigma_\gamma}$ and $s = 0$, Chen et al. managed to find the shifted Metropolis function \eqref{eq:shifted-metro-func}, after applying the linear combination to the Gaussian transition weight $\gamma_G(\omega)$ with the same $\omega_\gamma(x), \sigma_\gamma(x)$ parameters as in \eqref{eq:lin-comb-alpha},
\begin{equation} \label{eq:gamma_M-from-lin-comb-of-G}
    \gamma_M(\omega) = \int_{\beta\sigma^2 / 2}^\infty g(x)\gamma_G(\omega) \dd x.
\end{equation}
But while this leads to better mixing than the Gaussian case, it has some problems of its own the moment we want to implement it. 

The source of the problems is due to the fact that while $\gamma_M$ is Lipschitz continuous, it has a kink at $\omega = -\beta\sigma^2 / 2$ (and the unshifted variant at $\omega = 0$), see \hyperref[fig:transition-weights]{Fig.~\ref*{fig:transition-weights}}. Although the incurring approximation errors are only subleading in gate and ancilla costs for the algorithm, they seem to be unnecessary costs that can be easily avoided. This can matter for a near-term implementations prior full fault-tolerance: even for the humble system sizes we can classically simulate \hyperref[sec:na-transiton-weights]{Sec.~\ref*{sec:na-transiton-weights}}, it can save significant amount of phase estimating ancillas to design smoother transition weights. Some comments on how the kink of the Metropolis function affect the algorithm:

\begin{itemize}
    \item It leads to additional circuit synthesis costs, since in the quantum algorithm at one point we have to apply a rotation onto an ancilla qubit based on the value of $\gamma(\omega)$ that is encoded in the energy register, c.f. \hyperref[alg:diss]{Alg.~\ref*{alg:diss}} and \hyperref[circ:u-diss]{Fig.~\ref*{circ:u-diss}}. There are multiple ways to do this, but one is to use QSP, which has to approximate $\gamma_M$ via some polynomials. Due to the kink however it requires a higher degree polynomial for success, and thus a higher circuit complexity. Chen et al. notes \cite[footnote 33.]{chen2023quantum} that we could do this also piecewise, if we check the energy register via a comparator circuit block (to decide between left and right of the kink) and then apply QSP on smoother pieces of functions. If the kink is not at $\omega = 0$, as it generally is not, then such a comparator circuit would need to check more bits in the register, not only the sign bit, hence leading to a higher cost than anticipated, and rendering it as an impractical solution.
    \item The next implication is more important significant: since there is an integral over $\omega$ in the CKBG and CKG Lindbladians, we would have to approximate this with a quadrature sum in the quantum computer. A classical result of Fourier analysis \cite[Chapter 2.]{stein2011fourier} states that if a function $f$ is $k$ times continuously differentiable with $f^{(k)}$, its Fourier transform decays as $\hat{f}(t) = \mathcal{O}(|t|^{-(k+1)})$. The Metropolis $\gamma_M$ has a discontinuous first derivative at the kink, and thus its Fourier transform decays only as $\mathcal{O}(1 / t)$. Of course the convolution with the Gaussian filters with width $ \sigma^{-1}$ smoothen this kink and improves the scaling, but the algebraic prefactor persists $\mathcal{O}(\frac{e^{-\sigma^2 t^2}}{\sigma^2 t^2})$. Accordingly, this algebraic tail requires a larger truncation range and a finer grid (more estimating ancilla qubits) to achieve the same precision as an analytic transition weight. The more estimating ancillas the more exponentially growing controlled Hamiltonian evolutions.
    \item Finally, the existence of the kink is tightly connected to having a constant piece of the Metropolis function that equates to 1 for $\omega \leq \beta\sigma^2 / 2$. When passing to the time-domain representation of the Lindbladian via a Fourier transform, this constant piece produces a $\delta(t)$ singularity in the integral kernel. In practice this singularity has to be tamed by some regularization technique which incurs more costs as well.
\end{itemize}

In turns out that we can have the cake and eat it too, at least approximately. We can have close to optimal transitions while not stumbling into all the additional implementation costs. The solution is to smoothen the kink of the Metropolis function $\gamma_M$, which eliminates the first two previously mentioned problems. After a long battle of trying to improve on \cite{chen2023quantum}'s way of regularizing the singularity in the time domain, it seems that the incurring additional costs cannot be made better. The remnants of the battle can be found in [REF to APPENDIX] \todo{appendix where parameter a is back in the lin comb.}.

In order to design a better transition function $\gamma$, we will make use of the linear combination that was introduced in \eqref{eq:lin-comb-alpha}. The key is to choose $s \neq 0$ to shift the lower limit of the integration away from the kink, in a tuneable manner, i.e. the larger $s$ is the stronger the smoothing but also the more restrictive the transition rates become. Explicitly writing out $\gamma_G$ and $g(x)$ yields,
...
\end{equation}
The resulting the transition function cleanly separates into the Metropolis function $\gamma_M$ with an additional factor which depends on $s, \beta, \sigma$:
...
\smallskip
\label{sec:ckg-kossakowski}
\section{Kossakowski matrices}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{plots/kossakowski_heatmap_ckg.pdf}
    \caption[CKG Kossakowski matrix $\alpha_{\nu_1, \nu_2}$.]{%
        CKG Kossakowski matrix $\alpha_{\nu_1, \nu_2}$ on the rescaled Bohr
        grid for a 1D Heisenberg chain of $n = 3$. 
        \emph{Left:} Davies (GNS DB) at
        $\beta = 0.5$. It has only secular entries $\nu_1 = \nu_2.$
        \emph{Centre:} KMS DB at the same intermediate inverse temperature
        $\beta = 0.5$. It has a
        skew-symmetric off-diagonal pattern at $\sigma = 1/\beta^\sharp$.
        \emph{Right:} KMS DB at the colder $\beta = 1.0$;
        the off-diagonals sharpen and the mass concentrates closer to the
        $\nu_1 = \nu_2 = -1/\beta^\sharp$ Gaussian centre as
        temperature drops.}
    \label{fig:kossakowski-ckg}
\end{figure}
...
\smallskip
\label{sec:ckg-time-domain}
\section{Time, time, time} \todo{add norms and LCU subnormalizations here.}
...
\begin{equation} \label{eq:b_minus}
    b_-(t) = \frac{2\sqrt{\pi}}{\beta \sigma}e^{\beta^2\sigma^2/8}\left[\frac{1}{\cosh(\frac{2\pi t}{\beta \sigma})}\ast_t \sin(-\beta \sigma t) e^{-2t^2}\right] \quad\text{with}\quad \|b_-\|_1 \leq 1,
\end{equation}
where $\ast_t$ denotes the convolution over the variable $t$, 
$$
    (f \ast_t g)(t) := \int_{-\infty}^{\infty} f(s) g(t - s)\dd s.
$$
The upper bound $\|b_-\|_1 \leq 1$ was derived in \cite[Cor. III.1]{chen2023efficient} via Hölder's inequality. Importantly, $b_-(t)$ is a real function with support over $t = \mathcal{O}(\beta \sigma) = \mathcal{O}(1)$ due to the width of the $1 / \cosh$ kernel (using $\sigma = 1 / \beta$). The time evolution of the outer integral is $e^{\pm \beta H t / \sigma}$, which implies Hamiltonian simulation time of $\mathcal{O}(\beta)$.
...
\todo{check this one more time when rereading}
\begin{equation} \label{eq:b_plus-gauss}
    b_+(t) = \frac{ \beta\sigma_\gamma}{\pi\sqrt{\pi}}e^{-4t^2\beta \omega_\gamma - 2it\beta\omega_\gamma} \quad\text{with}\quad \|b_+\|_1 = \frac{\sigma_\gamma}{2\pi} \sqrt{\frac{\beta}{\omega_\gamma}}.
\end{equation}
...
\smallskip
\label{sec:ckg-quadrature}
\section{Quadrature errors}
...
\smallskip
\label{sec:ckg-trotter}
\section{Hamiltonian Trotterization}
...
\smallskip
\label{sec:ckg-gensplit}
\section{Generator splitting}
...
\medskip
\label{sec:ckg-coh-diss-split}
\subsection{Coherent--Dissipative.}
...
For this reason, higher-order coherent-dissipative product formulas do not improve the asymptotic end-to-end scaling in our setting. If the weak measurement scheme were replaced by some more accurate variant, then the splitting analysed above could be lifted to a matching higher-order formula.
...
\begin{figure}[ht]
  ...
  %
  % Cell layout (rows 1-3 have 15 cells, row 4 has 16):
  %   1:  \lstick                                  (left label)
  %   2:  wire decl                                (\qw or \qwbundle{r})
  %   3:  L_0 (row 1) / qw (rows 2-4)              CoherentStep box START
  %                                                opening rotation (FIRST in time)
  %   4:  octrl (row 1) / W (rows 2-4)             INNER (Repeat d, open-ctrl) START
  %                                                signal slot, fires on QSP=|0>
  %   5:  R_j (row 1) / qw                         INNER (open-ctrl) END
  %   6:  ctrl (row 1) / W^dagger (rows 2-4)       INNER (Repeat d, closed-ctrl) START
  %                                                signal slot, fires on QSP=|1>
  %   7:  R_{d+j} (row 1) / qw                     INNER (closed-ctrl) END
  %   8:  meter (rows 1-3) / qw (row 4)            CoherentStep box END (no W^-d tail)
  %   9-11: ancilla-reuse transition cells          (extra &[8pt] gap after cell 8)
  %  12-15: DissipativeStep
  %
    \lstick{$\ket{0}_{\mathrm{QSP}}$}
      & \qw
      & \gate{R(\theta_0,\phi_0,\lambda_0)}
        \gategroup[4,steps=6,style={rounded corners,fill=blue!6,inner xsep=0pt,inner ysep=6pt},
                   background,label style={label position=above,yshift=0.30cm,anchor=south}]
          {\scriptsize \textsc{CoherentStep}: $\ee^{-\ii\delta B_a} + \mathcal{O}(\delta^2)$}
      & \octrl{1}
        \gategroup[4,steps=2,style={dashed,rounded corners,inner sep=3pt},
                   background,label style={label position=below,yshift=-0.55cm,anchor=north}]
      ...
      & \qw
      & \qw
        \gategroup[4,steps=4,style={rounded corners,fill=red!6,inner sep=6pt},
                   background,label style={label position=above,yshift=0.30cm,anchor=south}]
          {\scriptsize \textsc{DissipativeStep}: $\ee^{\delta\mathcal{L}_{a,\mathrm{diss}}} + \mathcal{O}(\delta^2)$}
      & \gate{Y_\delta}
      & \octrl{1}
      & \meter{}
      \\
  ...
  \caption{Algorithm~\ref*{alg:main}: one $\delta$-step of the CKG Lindbladian
    ...
    \ee^{\delta\mathcal{L}_{a,\mathrm{diss}}}+\mathcal{O}(\delta^2)$ for a
    fixed jump $A_a$, split into a GQSP \emph{coherent
    block} (blue, walk operator $W = R_T\,U_{B_a}$ as in
    Fig.~\ref{circ:gqsp}), and a weak-measurement
    \emph{dissipative block} (red, $U_{\mathrm{diss}}$).
    $\textsc{Prep}'_\pm/\textsc{Prep}_\pm^\dagger$ are absorbed into $U_{B_a}$ inside each $W$ call (Fig.~\ref{circ:U_B_a}). Mid-circuit measurements mark ancilla reuse: $q_{\mathrm{QSP}}, T_\pm\!\to q_\delta, \Omega$.}
  \label{circ:algorithm-delta-step}
\end{figure}


It is time to assemble the subroutines discussed in the preceding subsections into a complete quantum algorithm for Gibbs state preparation via the CKG Lindbladian \eqref{eq:ckg-L}. The variant we present is the weak measurement + GQSP scheme, whose backbone is given by \cite[Thm. III.1]{chen2023quantum}. This is a deliberate choice for a near-term hardware realisation. The same paper from Chen et al. also give a \emph{compressed} Lindbladian simulator \cite[Thm. III.2]{chen2023quantum}, inspired by the techniques in \cite{cleve2016efficient}. Although, this variant is asymptotically close to optimal, i.e. its $\varepsilon$-dependence is $\mathrm{polylog}(1/\varepsilon)$ rather than simply $1 / \varepsilon$, it achieves this at the cost of a more complex circuit construction: involving oblivious amplitude-amplification and post-selection, together with an ancilla register that grows logaritmically in mixing time and thus in $\beta$. Due to the skyrocketing costs even in classical simulations, we settle to mainly use and analyse the first variant of the two.

The resulting procedure, Algorithm~\eqref{alg:main}, simulates the continuous-time Markov semigroup $\ee^{t \mathcal{L}}$ by repeated application of small $\delta$-step channels that approximate $\ee^{\delta \mathcal{L}_a}$ for each jump $a\in\mathcal{A}_a$. Its structure draws on three key ingredients: the \emph{weak measurement simulation scheme} of \cite{chen2023quantum}, which implements the dissipative channels $\ee^{\delta \mathcal{L}_{a, \text{diss}}} + \mathcal{O}(\delta^2)$; the \emph{coherent term} $B_a$ \eqref{eq:coherent-time-domain} introduced in \cite{chen2023efficient}, that makes the CKG generator $\mathcal{L}$ exactly KMS detailed balanced at the generator level; and \emph{Generalized Quantum Signal Processing} (GQSP) from \cite{motlagh2024generalized} introduced in \hyperref[sec:prelim-QSP]{Sec.~\ref*{sec:prelim-QSP}}, which provides a near-optimal method to realize the unitary $\ee^{-\ii \delta B_a} + \mathcal{O}(\delta^2)$ from the block encoding $U_{B_a}$ of $B_a / \alpha$.
...
\begin{itemize}
    \item The system register $S$ can be initalised in any arbitrary state $\rho_0$, due to the primitivity of $\mathcal{L}$: $\rho_\beta$ is the unique fixed point and the semigroup is ergodic, thus any initial state converges to $\rho_\beta$ at the same asymptotic rate. In practice however, it is ideal to choose a state that has a reasonable overlap with $\rho_\beta$ (and is not too costly to prepare), e.g. the maximally mixed state $\idm / d$, to eliminate part of the initial sampling steps, related to the factor $1/\sqrt{\lambda_{\min}(\rho_\beta)}$. (An even better choice is simply a higher temperature Gibbs state that is easier to prepare than a colder temperature one, but that has a significantly higher cost to it.)
    \item From the mixing time definition \eqref{eq:mixing-time-defi} and its spectral upper bound \eqref{eq:quantum-spectral-bound}, we know that after evolving some initial state $\rho_0$ for a total time $t = t_{\mathrm{mix}}(\mathcal{L})\,\log(2/\varepsilon)$ it is guaranteed to have $\|e^{t\mathcal{L}}(\rho_0) - \rho_\beta\|_1 \leq \varepsilon$. 
    \item For each $\delta$ step of the outer-loop we perform a sequential sweep over all $M_\mathcal{A} = |\mathcal{A}|$ jumps. This is a Lie-Trotter splitting of $e^{\delta\mathcal{L}}$ to $\Phi_\mathcal{A} := e^{\delta\mathcal{L}_{M_\mathcal{A}}} \circ \cdots \circ e^{\delta\mathcal{L}_1}$, introducing approximation errors of $\mathcal{O}(\delta^2)$ \eqref{eq:gen-split-approx-error}, while perturbing the real part of the spectrum only at $\mathcal{O}(\delta^3)$ \eqref{eq:gen-split-imaginary-delta2-errors}.
    \item In the inner-loop for a fixed $a$ we split the generator further as $\mathcal{L}_a = \mathcal{L}_{a,\mathrm{coh}} + \mathcal{L}_{a,\mathrm{diss}}$, introducing an error to the channel $\ee^{\delta \mathcal{L}_a}$ of order $\mathcal{O}(\delta^2)$, depending on the commutator between the coherent and dissipative parts \eqref{eq:gen-split-coh-diss}.
...
\caption{\textsc{CoherentStep}: Hamiltonian simulation of
         $e^{-\mathrm{i}\delta B_a}$ via block encoding and GQSP}
\label{alg:coh}
\begin{algorithmic}[1]
\small
\Require System register $S$, jump index $a$, step size $\delta$,
         GQSP degree $d$ chosen for target precision~$\varepsilon_\mathrm{QSP} = \mathcal{O}(\delta^2)$, often $d = 1$ suffices
         \eqref{eq:qsp-degree}
\Ensure  $S$ evolved by $e^{-\mathrm{i}\delta B_a}$ up to precision of $\mathcal{O}(\delta^2)$
\Statex
\State Ancillas:
  outer time register $T_-$ ($r_-$ qubits),
  inner time register $T_+$ ($r_+$ qubits),
  GQSP signal qubit $q_{\mathrm{QSP}}$
\Statex
\Statex \textit{-- Offline (classical) precomputation --}
\State Compute the Jacobi--Anger Laurent truncation
  $L_d(\ee^{\ii\theta}) = \sum_{k=-d}^{d}(-\ii)^k J_k(\delta\alpha)\,\ee^{\ii k\theta}$
  with $\alpha = \lVert b_-\rVert_1\,\lVert b_+\rVert_1$, and $P(z) = z^d L_d(z)$ of degree $2d$. Construct the complementary polynomial $Q\in\mathbb{C}[z]$
  with $|P(z)|^2 + |Q(z)|^2 = 1$ on $\mathbb{T}$ via
  \cite[Alg.~1]{berntson2025complementary} (via FFT).
  Extract the $2d{+}1$ rotation triples $\{(\theta_0,\phi_0,\lambda_0)\}\cup\{(\theta_j,\phi_j)\}_{j=1}^{2d}$ from $(P,Q)$ via \cite[Alg.~1]{motlagh2024generalized}.
\Statex
\Statex \textit{-- Quantum step. Block encoding $U_{B_a}$ of
  $B_a/\alpha$~\eqref{eq:B-discr} --}
\State Define the walk operator
  $W := R_T\,U_{B_a}$, where
  $R_T = 2\ket{0}\!\bra{0}_{T_-T_+} - I$
  is the joint reflection around the $\ket{0}$ subspace of $(T_-, T_+)$,
  and $U_{B_a}$ is the nested-LCU block encoding of
  Fig.~\ref{circ:U_B_a}:
  \begin{itemize}
    \item[] $\textsc{Prep}'_-:\ |0\rangle \to \displaystyle\sum_{\bar t}\,
            \frac{b_-(\bar t)}{\sqrt{|b_-(\bar t)|}}\,
            \frac{\sqrt{t_0}}{\sqrt{\alpha_-}}\,|\bar t\rangle$
            \quad(\eqref{eq:b_minus}, $\alpha_- = \lVert b_-\rVert_1$)
    \item[] $\textsc{Prep}_-^\dagger:\
            \displaystyle\sum_{\bar t}\sqrt{\frac{t_0\,|b_-(\bar t)|}{\alpha_-}}\,|\bar t\rangle \to |0\rangle$
    \item[] $\textsc{Prep}'_+:\ |0\rangle \to \displaystyle\sum_{\bar t'}\,
            \frac{b_+(\bar t')}{\sqrt{|b_+(\bar t')|}}\,
            \frac{\sqrt{t_0'}}{\sqrt{\alpha_+}}\,|\bar t'\rangle$
            \quad(\eqref{eq:b_plus-s-eta}, $\alpha_+ = \lVert b_+\rVert_1$)
    \item[] $\textsc{Prep}_+^\dagger:\
            \displaystyle\sum_{\bar t'}\sqrt{\frac{t_0'\,|b_+(\bar t')|}{\alpha_+}}\,|\bar t'\rangle \to |0\rangle$
  \end{itemize}
  so that the amplitudes multiply to the signed kernel
  $b_\pm(\bar t)\,t_0/\alpha_\pm$.
  $U_{B_a}$ then satisfies
  $\langle 0|_{T_-}\!\langle 0|_{T_+}\,U_{B_a}\,|0\rangle_{T_-}\!|0\rangle_{T_+} = B_a/\alpha$
  on $S$.
\Statex
\Statex \textit{-- GQSP: implement $e^{-\mathrm{i}\delta B_a}$ from $W$ --}
\State Initialise $q_{\mathrm{QSP}}$ in $|0\rangle$
\State Apply the opening rotation $L_0 = R(\theta_0,\phi_0,\lambda_0)$ on $q_{\mathrm{QSP}}$
\For{$j = 1, 2, \dots, 2d$}
  \Comment{$2d$ controlled signal slots}
  \If{$j \le d$}
    \State Apply $\Lambda(W)$ on $(q_{\mathrm{QSP}}; T_-, T_+, S)$
           \Comment{controlled $W$ fires on $q_{\mathrm{QSP}} = |0\rangle$}
  \Else
    \State Apply $\Lambda'(W)$ on $(q_{\mathrm{QSP}}; T_-, T_+, S)$
           \Comment{controlled $W^\dagger$ fires on $q_{\mathrm{QSP}} = |1\rangle$}
  \EndIf
  \State Apply $R(\theta_j,\phi_j,0)$ on $q_{\mathrm{QSP}}$
\EndFor
\State Measure $q_{\mathrm{QSP}}, T_-, T_+$ and post-select on
         $\ket{0}_{q_{\mathrm{QSP}}}\!\ket{0}_{T_-}\!\ket{0}_{T_+}$
  \State \Return $S$ evolved by $\ee^{-\ii\delta B_a}$ up to
         $\mathcal{O}(\varepsilon_\mathrm{QSP})$, and thus up to $\mathcal{O}(\delta^2)$.
\end{algorithmic}
\end{algorithm}
...
\begin{figure}[ht]
  ...
  \begin{quantikz}[row sep=0.9cm, column sep=0.42cm]
  % ==================================================================
  % Row 1:  q_gamma  (Boltzmann ancilla; rotated in cell 8)
  % ==================================================================
...
We can now sum up the total cost of \hyperref[alg:main]{Algorithm~\ref*{alg:main}}. The outer loop runs for $L = \lceil t_\text{mix}(\mathcal{L})\,\log(2/\varepsilon)/\delta\rceil$ steps and each step performs one sweep over the $M_\mathcal{A}$ jumps of $\textsc{CoherentStep}$ + $\textsc{DissipativeStep}$. The weak measurement primitive is first-order accurate in $\delta$ so the per-step error $\mathcal{O}(\delta^2)$ accumulates to $\mathcal{O}(t_\text{mix}\,\delta)$ over the full evolution \eqref{eq:weak-meas-euler}. Demanding $t_\text{mix}\,\delta \lesssim \varepsilon$, i.e. that the weak measurement baseline meett the target trace distance accuracy, sets
$$
\delta = \Theta(\varepsilon/t_\text{mix}), 
\qquad
L = \Theta(t_\text{mix}^{\,2}/\varepsilon).
$$
We also set the GQSP degree $d$ (most of the time $d = 1$) such that its per-step error in $\textsc{CoherentStep}$, $\varepsilon_\mathrm{QSP} = \mathcal{O}(\delta^2)$ is of the same order as of the weak measurement.

If we convert the block-encoding queries into total Hamiltonian simulation time, then we see that each call to $U_\mathrm{diss}$ carries an OFT-controlled evolution length $\mathcal{O}(\sigma\sqrt{\log(1/\varepsilon)})$ set by the Gaussian filter truncation, and each call to $U_B$ carries a maximum simulation time $\mathcal{O}(\beta\log(1/\varepsilon))$ set by the outer $T_-$ window, both established in the previous paragraphs. The dominant $\beta$ comes from the coherent term.
Combining the $\mathcal{O}(t_\text{mix}^{\,2}/\varepsilon)$ dissipator and coherent calls with these per-call Hamiltonian evolution times gives
\begin{equation} \label{eq:ckg-cost}
    \textbf{Ham. sim. time} \;=\; \tilde{\mathcal{O}}\!\bigl(|\mathcal{A}|\beta\,t_\text{mix}^{\,2}/\varepsilon\bigr),
\end{equation}
where the $\tilde{\mathcal{O}}$ hides $\mathrm{polylog}(\beta\|H\|/\varepsilon)$ factors from: the OFT register inside $U_\mathrm{diss}$, the dissipator quadrature, and the GQSP block of $U_B$. This is the cost we should expect in our implementations of \cite[Theorem~III.1]{chen2023quantum} via the weak measurement + GQSP scheme of \hyperref[alg:main]{Alg.~\ref*{alg:main}}. The only additional factor we see here compared to Chen et al. is the number of jumps applied $|\mathcal{A}|$ due to the jump-wise generator splitting we use instead of the block-encoded sum of jumps, see \hyperref[sec:ckg-jumpwise-split]{Sec.~\ref*{sec:ckg-jumpwise-split}}. 

On a fault-tolerant quantum computer that has an abundance of logical qubits, we could do better than a scaling linear in $1 / \varepsilon$. The follow-up algorithm described in \cite[Theorem~III.2]{chen2023quantum}, manages to \emph{compress} these weak measurement gadgets via many copies, amplitude amplification and post selection (and assuming block encoding of the sum of jumps is also no problem), yielding a polylog-$\varepsilon$ scaling,
\begin{equation} \label{eq:ckg-cost-compressed}
    \textbf{Compressed Ham. sim. time} \;=\; \tilde{\mathcal{O}}\!\bigl(\beta\,t_\text{mix}\,\mathrm{polylog}(1/\varepsilon)\bigr).
\end{equation}

Indeed, the price is paid by the ancilla budget and circuit complexity: the compressed simulator uses $\mathcal{O}((c + \log((t_\text{mix}+1)/\varepsilon))\,\log((t_\text{mix}+1)/\varepsilon))$ resettable ancillas, substantially more than the $c + 1$ in the simpler weak measurement based algorithm. Nevertheless, aside from circuit-level details, the compressed version almost achieve optimal scaling. For a Lindbladian $\mathcal{L}$ whose coherent term and Lindblad jumps are accessed via LCU block encodings, the best known algorithm that simulates $\ee^{t\mathcal{L}}$ to a $\varepsilon$ precision in diamond-norm, scales as $\tilde{\mathcal{O}}\bigl(t\,\mathrm{polylog}(t/\varepsilon)\bigr)$, i.e. almost linear in $t$ and polylogarithmic in $\varepsilon$ \cite{cleve2016efficient,li2022simulating}. The compressed algorithm is inspired by \cite{cleve2016efficient}, hence the similar scaling. Up to now all algorithms we mentioned were of first-order Lindblad simulations. Since then a higher-order Lindblad simulator from \cite{li2022simulating} achieved the same almost optimal complexity but without compressed encodings, using simple block encoding primitives and some amplitde amplifications. However, it is better suited for constructions where each Lindblad jump is separately block encoded, which is why \cite{ding2025efficient} chooses it as their simulation technique, see \hyperref[sec:dll]{Sec.~\ref*{sec:dll}}.

Some open questions are worth restating here from \cite{li2022simulating}, as it puts these Lindbladian simulation results in perspective. Hamiltonian simulations with block encoding and QSP techniques can be done in $\mathcal{O}(t + \text{polylog}(1 / \varepsilon))$ \cite{low2019hamiltonian}, which is optimal \cite{berry2015hamiltonian}. If such an additive complexity is possible for Lindblad simulations is currently unclear. An obstacle is that QSP techniques has not yet been generalized to superoperators. The other question is about practicality for the near-term quantum computers. 
In \cite{childs2018toward} the authors found that Trotter based Hamiltonian simulations are surprisingly competitive with their LCU and QSP based counterparts. Since no such research has been done to compare the asymptotically better-looking LCU based Lindblad simulators \cite{cleve2016efficient,li2022simulating} with the Stinesrping dilation based techniques \cite{childs2016efficient,kliesch2011dissipative} (and the weak measurment scheme), we cannot be sure which one works better for near-terms.

For quantum Gibbs state preparation specifically, there is another factor that is inherently part of the complexity, namely the inverse temperature $\beta$. The Gibbs state $\rho_\beta = \mathrm{e}^{-\beta H}/Z$ is a smooth matrix function of $H$ and with a sensitivity argument from \cite[Prop.~G.5]{chen2023quantum}, it is known that for any algorithm that prepares $\rho_\beta$ to some accuracy requires a total Hamiltonian simulation time of $\Omega(\beta)$. Otherwise, it would not be possible to distinguish $\rho_\beta$ from $\rho_{\beta + \mathcal{O}(1)}$. We can also udnerstand $\beta$'s role a bit more intuitively as well. At some $\beta$ the thermal energy scale is $1 / \beta$, so the natural unit of time in which a system equilibrates with its bath is $\beta$. A quantum sampler is then required to resolve energy differences on this $1 / \beta$ scale: either explicitly via QPE \cite{temme2011quantum} / OFT \cite{chen2023quantum}, or implicitly in the coherent term $B$ approximations of CKG.

\smallskip
\label{sec:parent-ham}
\section{Parent Hamiltonian} Beyond its role as a design principle for the algorithm, KMS detailed balance carries another analytical payoff that we have already touched upon: the similarity transformation leads to a \emph{Hermitian} quantum discriminant $\mathcal{D}(\rho_\beta, \mathcal{L})$ \eqref{eq:discriminant}. This transformation preserves the spectrum of the Lindbladian $\mathcal{L}$, which also keeps it negative semi-definite. Hence, to define a PSD parent Hamiltonian, we flip the signs, 
...
\newpage
\chapter{DLL Framework}
\label{sec:dll}
The CKG construction we analysed in the previous sections provided the breakthrough for exactly detailed balanced quantum Gibbs sampling. This is achieved by letting a \emph{continuum} of jumps $A(\omega)$ -- each parametrized over the energies $\omega$ in \eqref{eq:ckg-L} -- nudge the system coherently towards the Gibbs state. To keep it an efficient algorithm Chen et al. used Gaussian filters $\hat{f}$ to smoothen phase estimations, eliminating the need for precise (and costly) Bohr frequency resolutions. Shortly after \cite{chen2023efficient}, another version of this KMS detailed balanced construction was put forward by Ding et al. \cite{ding2025efficient}, which we will refer to as the \emph{DLL framework.} The structural difference is quite sharp: DLL replaces the continuum $\int \dd\omega\, \gamma(\omega) (\,\dots)$ by a finite set $\{L_a\}_{a\in\mathcal{A}}$ only indexed by the jump set $\mathcal{A}$, albeit with elaborate costs in the coherent term construction and block encodings. The motivation is purely algorithmic: a finite jump set opens the door to higher-order Lindbladian simulators such as the one in \cite{li2022simulating}, which the CKG continuum does not directly admit (hence the weak measurement scheme \hyperref[sec:prelim-weak-meas]{Sec.~\ref*{sec:prelim-weak-meas}}). The idea that makes this happen, is to find filter functions in the time-domain $f(t)$ that directly map to the right (skew-symmetric) Bohr contributions of the jump operator $A$, completely omitting the approximation middle-step via the $\omega$ continuum of CKG. Definitely an interesting promising direction to take, but a priori it is not clear if it leads to a better Gibbs sampler algorithm in a whole. In this section we recast DLL in our notation, contrast it with CKG at the level of Kossakowski matrices, coherent-term scaling, and discuss its asymptotic block-encoding costs. In the numerical section, we also compare their mixing times with the CKG framework, to see if there the structural difference leads to faster thermalization. \todo{REF to sec}

\smallskip
\label{sec:dll-discrete-jumps}
\section{From continuum to discrete jump set} The CKG construction starts from an ergodic set $\{A^a\}_{a\in\mathcal{A}}$, builds the OFT $\hat{A}^a(\omega)$ via \eqref{eq:oft}, and integrates against $\gamma(\omega)$ over all $\omega \in \mathbb{R}$. The same Bohr decomposition
$A^a = \sum_{\nu\in B_H} A^a_\nu$ (e.g. \eqref{eq:smoothed-jump}) underlies both frameworks, but DLL takes a different route: it picks $\gamma(\omega) = \delta(\omega)$, collapsing the integral to a single jump per index $a$, and then absorbs all spectral resolution into the filter itself. Concretely, given a single weight function $q^a:\mathbb{R}\to\mathbb{C}$ with the conjugate symmetry $q^a(-\nu) = \overline{q^a(\nu)}$ ($q^a$ structurally analogous to $\hat{f}$ in \eqref{eq:gaussian-filter-w}), the DLL Lindblad jumps are defined as:
\begin{equation} \label{eq:dll-jump}
    L_a \;:=\; \sum_{\nu \in B_H} q^a(\nu)\, e^{-\beta\nu/4}\, A^a_\nu
    \;=\; \int_{-\infty}^{\infty} f^a(t)\, A^a(t)\, \dd t,
\end{equation}
with 
$$
f^a(t) := \frac{1}{2\pi}\int_{-\infty}^{\infty} q^a(\nu)\, e^{-\beta\nu/4}\, e^{-\ii\nu t}\, \dd\nu,
$$
and $A^a(t) = e^{\ii H t} A^a e^{-\ii H t}$ is the Heisenberg evolution as in CKG \eqref{eq:oft}. In CKG language, this is \emph{exactly} the OFT $\hat{A}^a(\omega)$ evaluated at $\omega = 0$ with filter $f$. The set of jumps is then $\{L_a\}_{a\in\mathcal{A}}$ instead of $\{\sqrt{\gamma(\omega) A^a(\omega)}\}$. The continuum $\int \dd\omega\, \gamma(\omega)(\,\dots\,)$, or its implemented discretized sum $\sum_{\bar{\omega}} \gamma(\bar{\omega})(\,\dots\,)$, that runs through the entire CKG Lindbladian reduces to a single Lindblad term per $a$:
\begin{equation} \label{eq:dll-L}
    \mathcal{L}_\text{DLL}(\cdot) \;=\; -\ii[G,\,\cdot\,] \;+\; \sum_{a\in\mathcal{A}}\Big( L_a (\cdot) L_a^\dagger \;-\; \tfrac{1}{2}\{L_a^\dagger L_a,\,\cdot\,\}\Big).
\end{equation}
The KMS detailed balance condition on each $L_a$ becomes the structural identity $\Delta_{\rho_\beta}^{-1/2}(L_a) = L_a^\dagger$, which in our notation is 
\begin{equation} \label{eq:dll-jump-self-adjoint}
    \sum_{\nu \in B_H} q^a(\nu)\, e^{+\beta\nu/4}\, A^a_\nu \;=\; \Big(\sum_{\nu\in B_H} q^a(\nu)\, e^{-\beta\nu/4}\, A^a_\nu\Big)^\dagger,
\end{equation}
and is automatic when $A^a = (A^a)^\dagger$ and $q^a$ has the symmetry above. Similarly to the CKG coherent term, the coherent term $G$ is again uniquely determined and fixed by the dissipative part of the Lindbladian. By \cite[Thm. 10]{ding2025efficient} the coherent term satisfies $G = -\ii\,\tanh\!\circ\!\log(\Delta_{\rho_\beta}^{1/4})\bigl(\tfrac{1}{2}\sum_a L_a^\dagger L_a\bigr)$, which after substituting \eqref{eq:dll-jump} yields the Bohr-pair sum (similar but not quite to \eqref{eq:B-bohr-domain}):
\begin{equation} \label{eq:dll-G-bohr}
    G \;=\; \frac{1}{2\ii}\sum_{a\in\mathcal{A}}\sum_{\nu_1,\nu_2 \in B_H}\tanh\!\left(\frac{\beta(\nu_2-\nu_1)}{4}\right)\, \widehat{g}^{\,a}(\nu_1,\nu_2)\,(A^a_{\nu_2})^\dagger A^a_{\nu_1},
\end{equation}
with 
$$
\widehat{g}^{\,a}(\nu_1,\nu_2) := q^a(\nu_1)\,\overline{q^a(\nu_2)}\,e^{-\beta(\nu_1+\nu_2)/4}.
$$
Some structural observations should be noted before we discuss the consequences. The Bohr-pair sum in \eqref{eq:dll-G-bohr} is the same index range $\{(\nu_1,\nu_2)\in B_H\times B_H\}$ as what we find in CKG's \eqref{eq:B-bohr-domain}. The difference lies entirely in the prefactors: while CKG has a convolved kernel via $\alpha_{\nu_1,\nu_2}^{\textup{CKG}} = \int \gamma(\omega)\hat{f}(\omega-\nu_1)\hat{f}(\omega-\nu_2)\dd\omega$ \eqref{eq:kossakowski-smooth-metro}, DLL has a rank-1 outer product $\widehat{g}^a(\nu_1,\nu_2) = q^a(\nu_1)\overline{q^a(\nu_2)}\,e^{-\beta(\nu_1+\nu_2)/4}$. As now $\gamma(\omega) = \delta(\omega)$ all the spectral resolution is carried by the DLL filter $f$ itself. Thus, the Bohr-frequency content is resolved in two different ways: CKG works within the frequency register over the $\omega$-integral, while DLL compresses all of it into the operator $L_a$.

To make \eqref{eq:dll-jump} and \eqref{eq:dll-G-bohr} amenable to a finite-grid quadrature, $q^a$ must be a Gevrey-class function with a compactly supported window $w(\nu/S)$ (exactly the same kind of Gevrey-1/2 regularity that we proved for smooth Metropolis in \hyperref[prop:smooth-metro-gevrey]{Prop.~\ref*{prop:smooth-metro-gevrey}}). The support parameter $S>0$ truncates $q^a$ to $[-S, S]$ \todo{is this correct?} in a smooth manner. In \cite{ding2025efficient}, they propose two choices for the filter functions that we also adopt for our numerical simulations:
\begin{equation} \label{eq:dll-filters}
    \begin{split}
        q^a_G(\nu) &\;=\; e^{-(\beta\nu)^2/8}\,w(\nu/S), \\
        q^a_M(\nu) &\;=\; e^{-\sqrt{1+\beta^2\nu^2}/4}\,w(\nu/S),
    \end{split}
\end{equation}
which are a Gaussian-type and a Metropolis-type DLL filter respectively. At the qualitative level a familiar pattern to CKG: there the Metropolis weight 
$\gamma_M^{(s)}$ \eqref{eq:shifted-metro-func} outperforms the Gaussian one $\gamma_G$ \eqref{eq:gaussian-transition} due to the wider acceptance band for decreasing the energy of the system. For DLL the relation is similar, but the \emph{scaling} of the acceptance band of $q^a_M$ is unlike the rest. In the band $|\nu|\leq S/2$ the DLL Metropolis filter satisfies
$$
\widehat{f}^a_M(\nu) \;=\; q^a_M(\nu)\,e^{-\beta\nu/4} \;\approx\; \min\!\bigl\{1,\, e^{-\beta\nu/2}\bigr\},
$$
which keeps an $\mathcal{O}(1)$-wide acceptance window on the negative $\nu$ half, even at low temperature. The DLL Gaussian filter, by contrast, has $\widehat{f}^a_G(\nu) \propto e^{-(\beta\nu+1)^2/8}$, which is a Gaussian centred at $\nu=-1/\beta$ with width $\mathcal{O}(1/\beta)$. Hence, its acceptance window shrinks as $\beta$ grows. The CKG construction never escapes this $\mathcal{O}(1/\beta)$ scale: both $\gamma_G$ and $\gamma_M^{(s)}$ enter $\alpha^{\textup{CKG}}_{\nu_1,\nu_2}$ through the convolved OFT kernel $\hat{f}(\omega-\nu_1)\hat{f}(\omega-\nu_2)$ with $\sigma = 1 / \beta$, so the dominant Kossakowski block is an $\mathcal{O}(1/\beta)$-wide diagonal band for both. What the Metropolis weight changes inside CKG is merely the length of that band along the $\nu_1 = \nu_2$ diagonal, not its width. 

\smallskip
\label{sec:dll-kossakowski}
\section{Kossakowski matrix} The most transparent way to contrast the two frameworks is to compare their Kossakowski matrices on a common Bohr grid $B_H \subset \mathbb{R}$. For CKG $\alpha_{\nu_1,\nu_2}^{\textup{CKG}}$ from \eqref{eq:kossakowski-smooth-metro} is a fully populated, full-rank Hermitian PSD matrix whose off-diagonal mass decays as $e^{-(\nu_1-\nu_2)^2/(8\sigma^2)}$, hence as $\mathcal{O}(1 / \beta)$ if $\sigma = 1/\beta$, see \hyperref[fig:kossakowski-ckg]{Fig.~\ref*{fig:kossakowski-ckg}}. For DLL, on the otherhand,
\begin{equation} \label{eq:dll-kossakowski}
    \alpha_{\nu_1,\nu_2}^{\textup{DLL}} \;=\; \widehat{g}^a(\nu_1,\nu_2) \;=\; \bigl(q^a(\nu_1)\,e^{-\beta\nu_1/4}\bigr)\,\overline{\bigl(q^a(\nu_2)\,e^{-\beta\nu_2/4}\bigr)},
\end{equation}
is by construction a rank-1 outer product. This leads to the following structural consequences:
\begin{itemize}
    \item \textbf{KMS skew-symmetry} \eqref{eq:skew-symmetry} is automatic, since
    $$
    \alpha^{\textup{DLL}}(\nu_1,\nu_2) = \alpha^{\textup{DLL}}(-\nu_2,-\nu_1)\,e^{-\beta(\nu_1+\nu_2)/2}
    $$ 
    reduces to $q(-\nu) = \overline{q(\nu)}$, per definition.
    \item \textbf{The $(-, -)$ corner,} i.e. the block entries $(\nu_1, \nu_2)$ with both indices negative and $|\nu_i|\leq S/2$, is $\beta$-independent in the Metropolis-type filter $\widehat{f}^a_M(\nu)\to 1$ for $\nu\to-\infty$. In \hyperref[fig:kossakowski-dll]{Fig.~\ref*{fig:kossakowski-dll}} it becomes clear how this corner survives even in the teeth-chattering cold for the Metropolis case (bottom row), not so for the Gaussian case (top row).
\end{itemize}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{plots/kossakowski_dll_heatmaps.pdf}
    \caption[DLL Kossakowski matrix $\alpha_{\nu_1, \nu_2}$.]{DLL Kossakowski matrix $\alpha_{\nu_1, \nu_2}$ for
        a 1D Hiesenberg chain with $n = 3$, swept across
        $\beta \in \{0.25, 0.5, 1.0\}$ (columns).
        \emph{Top row:} DLL Gaussian filter - centred at
        $\nu_1 = \nu_2 = -1/\beta^\sharp$ that shrinks as $\beta$ grows,
        a super-exponential localisation in $\beta\nu$ that drives the
        low-temperature mixing collapse.
        \emph{Bottom row:} DLL Metropolis filter — the $(-,-)$ quadrant
        saturates to $\hat{f}(\nu\to-\infty) = 1$ while the $(+,+)$ corner
        is suppressed as $\exp(-\beta^\sharp(\nu + \nu'))$.}
    \label{fig:kossakowski-dll}
\end{figure}

Now, as promising as it sounds to keep a wider acceptance rate for lower temperatures with the DLL Metropolis filter, a priori it is not clear how that affects mixing times. Extra off-diagonal mass could in principle accelerate equilibration by coupling more Bohr-frequency pairs, but the Kossakowski matrix is only one ingredient of the spectral gap, especially as these off-diagonals feed into the coherent term $G$ or $B$ as well. There is no clear theoretical result on how or if off-diagonal terms in Kossakowski matrices speed up the mixing process. While we are in the dark theoretically, numerical simulations can still shed some light on the matter.In [REF] \todo{ref dll v ckg}, we compare spectral gaps and accurate mixing times of the DLL and CKG Lindbladians, and ... \todo{fill out after finalized plot results}

\smallskip
\section{Costs and scaling}
The most consequential implication of the rank-1 Kossakowski structure \eqref{eq:dll-kossakowski} lies in the block-encoding subnormalisation that the new coherent term $G$ acquires. To see this concretely, recall that each of these proposed quantum Gibbs samplers, either CKG (\hyperref[alg:coh]{Alg.~\ref*{alg:coh}}) or DLL, requires an LCU block encoding for $B$ or $G$ respectively. In the time domain the subnormalization factor of CKG's $B$ with Metropolis transitions is $\|b_-\|_1\,\|b_+\|_1 \;=\; \mathcal{O}\!\Bigl(\log\frac{\beta\|H\|}{\varepsilon}\Bigr)$, see \eqref{eq:coherent-time-domain}. For them the linear $\beta$ scaling of the total Hamiltonian simulation time enters through the truncation window of the quadrature sums approximating $B$, not through the LCU subnormalization. 

In the DLL case \eqref{eq:dll-G-bohr}, the situation is different. The block encoding of $G$ proposed by DLL is a double-LCU sandwich acting on both time registers (their time kernel could not be factorized into two Fourier transforms like in CKG). Due to the fact that DLL retains an $\mathcal{O}(1)$ scaling for their Kossakowski matrix \eqref{fig:kossakowski-dll} instead of the $\mathcal{O}(1 / \beta)$ in CKG \eqref{fig:kossakowski-ckg}, their block encoding necessarily picks up an additional $\beta$ factor, yielding $\alpha_G = \tilde{\mathcal{O}}(\beta S)$. But we also see the mechanism that would match them up: changing the scaling of the support window parameter $S$ from $\Omega(1)$ to $\mathcal{O}(1 / \beta)$. This would lead to the same asymptotic scaling for the DLL algorithm, the (-,-) corner of the Kossakowski matrix would shrink similarly with $\mathcal{O}(1/\beta)$ as CKG's. Currently, it is unclear if it is worth to pay this additional complexity for the DLL algorithm, because we do not know if their $\mathcal{O}(1)$ Kossakowski matrix would lead to better mixing times at low temperatures; our numerical analysis shows no sign of such a trend [REF]. \todo{ref} The caveat here is that there could be Hamiltonians, for which the slow decaying mode is positioned at larger Bohr frequencies -- further away from the diagonal -- where DLL shines, since CKG's Gaussian filters suppress contributions.

This scaling difference for the subnormalization directly appears in the Lindbladian simulation costs in terms of the total Hamiltonian simulation times as \cite[Thm. 19]{ding2025efficient}
\begin{align} \label{eq:dll-vs-ckg-cost}
\textbf{Total Ham. sim. time for DLL} &\;=\; \widetilde{\mathcal{O}}\bigl(\beta^2\,S\,t_\text{mix}\,\textup{polylog}(1/\varepsilon)\bigr), \\
\intertext{and setting this side by side with the CKG total (compressed) cost \eqref{eq:ckg-cost-compressed},}
\textbf{Total Ham. sim. time for CKG} &\;=\; \widetilde{\mathcal{O}}\bigl(\beta\,t_\text{mix}\,\textup{polylog}(1/\varepsilon)\bigr).
\end{align}

The asymptotic complexities \eqref{eq:dll-vs-ckg-cost} do not tell the whole story however, especially because we compared DLL to the compressed algorithm of CKG \cite[Thm.~III.2]{chen2023quantum} whose circuit is too complex for a near-term implementation due to the number of ancillas it requires. Indeed, there is a flip side to the DLL structure, that we foreshadowed in \hyperref[sec:prelim-weak-meas]{Sec.~\ref*{sec:prelim-weak-meas}}. Namely, that the finite jump set $\{L_a\}_{a\in\mathcal{A}}$ admits a \emph{higher-order Lindbladian simulators} that works without compressions from \cite{cleve2016efficient}, requiring only the block encoding ancillas and certainly some oblivious amplitude amplifications. The recently developed Lindblad simulator DLL proposes to use is based on the Duhamel series of \cite[Thm. 11]{li2022simulating}, that gives an $\varepsilon$-precise simulation of a Lindbladian 
$$
\mathcal{L}(\rho) \;=\; -\ii[G,\rho] \;+\; \sum_{j=1}^{m}\mathcal{D}_{L_j}(\rho)
$$
in 
$$
\widetilde{\mathcal{O}}\bigl(t\,\|\mathcal{L}\|_\textup{be}\,\log(t\|\mathcal{L}\|_\textup{be}/\varepsilon)\bigr) \quad\text{queries to } U_G, \{U_{L_j}\},
$$
with the block-encoding norm $\|\mathcal{L}\|_\textup{be} = \widetilde{\mathcal{O}}\bigl(\beta^2 S\,|\mathcal{A}|\bigr)$. Crucially, this is \emph{polylogarithmic} in $1/\varepsilon$, similar to the compressed CKG algorithm, but exponentially better than the weak measurement scheme's $\mathcal{O}(1 / \varepsilon)$ dependence. The reason CKG opted for compression is due to their continuum $\int\dd\omega\,\gamma(\omega)(\ldots)$ (even if discretized) cannot be separated from the block encoding. Although, \cite{li2022simulating} works by encoding sums of jumps $L_j$, the discretized $\bar{\omega}$ parameter of the CKG jumps $\sqrt{\gamma(\bar{\omega})} \hat{A}(\bar{\omega})$ can become very costly to block encode. In this sense, the weak measurement scheme and its compression are indeed much more natural constructions to simulate the CKG Lindbladian \eqref{eq:ckg-L}, while \cite{li2022simulating} fits well with the DLL Lindbladian \eqref{eq:dll-L}.