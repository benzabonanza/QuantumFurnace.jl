...
\label{ch:quantum-formalism}
\chapter{Quantum Formalism and Notation}
...
\smallskip
\subsection{Linear Combination of Unitaries.}\label{sec:prelim-LCU}
...
\end{equation}
i.e. $U_A$ is a block encoding of $A$ with subnormalisation $\alpha:= \|f\|_1$ \cite{low2019hamiltonian}. The subnormalisation is the price for packing a non-unitary $A$ inside a unitary: $U_A$ acts as $A / \alpha$ on the block projector $\ket{0^b}\!\bra{0^b}$ and as an unspecified completion on its orthogonal subspace, and all complexity statements about simulation times and success probabilities scale with $\alpha$.
...
Applying $U_A$ on $\ket{0^b}\otimes\ket{\psi}$ and \emph{post-selecting} the ancilla on $\ket{0^b}$ leaves, up to normalisation, the state $A \ket{\psi} / \alpha$ on the system, with success probability $\|A\ket{\psi}\|^2/\alpha^2$. When $\alpha \leq 1$, the block encoding can be taken as a deterministic implementation of $A$. In the case when $\alpha \geq 1$, however, the post-selection succeeds only with $\mathcal{O}(1 / \alpha^2)$, in which case LCU is \emph{not} deterministic. For bare applications of LCU, there are already remedies for this, e.g. a repeat-until-success procedure with $\mathcal{O}(\alpha^2)$ of repetitions, or the so-called oblivious amplitude amplifications which involves additional reflections. But in our thesis, an indeterministic LCU can only happen when we create a block encoding for a Hermitian operator $B$ and then directly transform it to a coherent evolution $\ee^{-\ii B t}$. This transformation goes hand in hand with block-encodings, which is the next subroutine we will introduce. There, it becomes clear what role the subnormalization factor $\alpha$ plays in the algorithm's complexity.

\smallskip
\subsection{Quantum Signal Processing.} \label{sec:prelim-QSP}
\begin{figure}[ht]
  \centering
  ...
  \begin{quantikz}[row sep=0.7cm, column sep=0.5cm]
    \lstick{$\ket{0}_\mathrm{QSP}$}
      ...
      & \qw
      & \octrl{1}
      \gategroup[3,steps=2,style={dashed,rounded corners,fill=black!6,inner sep=5pt},background,label style={label position=below,yshift=-0.45cm,align=center}]{\scriptsize \quad Repeat $d$ times\\\scriptsize $j = d{+}1,\dots,2d$}
      & \gate{R(\theta_j,\phi_j,0)}
      & \qw
  ...
  from the walk operator $W$ of \ref{eq:qsp-walk}.
  The first $d$ signal calls are controlled-$W$ and the last $d$
  are controlled-$W^\dagger$ (\cite[Thm. 6]{motlagh2024generalized}, $k = d$).
  Post-selecting both ancillas on $\ket{0}$ yields
  $\ee^{-\ii\delta A} + \mathcal{O}(\varepsilon_\mathrm{QSP})$.}
  \label{circ:gqsp}
\end{figure}

Without spoiling too much of the coming theoretical discussions, let us simply claim, that for a detailed balanced Lindbladian we need to apply a coherent evolution $\ee^{-\ii\delta A}$ with respect to a Hermitian operator $A$. Although the method we are about to introduce here is not the only possibility, it certainly is a good fit for our setup. 
...
What we want to apply to the system however is not purely $A$ but rather the coherent evolution $\ee^{-\ii\delta A}$. \emph{Generalized quantum signal processing} (GQSP) \cite{motlagh2024generalized} is the tool that transforms one into the other: interleaving controlled calls to an associated \emph{walk operator} -- consisting of $U_A$ and reflections -- with single-qubit rotations realises any polynomial transformation of $A$'s spectrum after post-selection. Choosing the polynomial $P$ as a good approximation to $\ee^{-\ii\delta\alpha\cos\theta}$ on the unit circle realises the Hamiltonian simulation unitary $\ee^{-\ii\delta A}$ up to a QSP truncation error. The circuit in \hyperref[circ:gqsp]{Fig.~\ref*{circ:gqsp}} shows a GQSP layer for an arbitrary degree $d$ polynomial approximation. 

Following \cite[Cor. 8.]{motlagh2024generalized}, define the walk operator as a composition of \textsc{Select} and a reflection around the block-encoding ancillas,
\begin{equation} \label{eq:qsp-walk}
    W \;:=\; \bigl(2\,\textsc{Prep}\ket{0}\!\bra{0}\,\textsc{Prep}^\dagger \,-\, I\bigr)\,\textsc{Select}.
\end{equation}
For each eigenvector $\lambda$ of $A$ with $A\ket{\lambda} = \lambda\ket{\lambda}$, the two-dimensional subspace containing $\text{PREP}\ket{0}\ket{\lambda}$ (and some other orthogonal state) is invariant under $W$. In the suitable orthonormal basis $W$ acts as the $\mathrm{SU}(2)$ matrix
$$
W_\lambda \;=\; \begin{pmatrix} \lambda/\alpha & \sqrt{1-(\lambda/\alpha)^2} \\[2pt] -\sqrt{1-(\lambda/\alpha)^2} & \lambda/\alpha \end{pmatrix},
$$
Its eigenvalues form the conjugate pair $\ee^{\pm\ii\theta}$ with 
$$
...
A GQSP circuit \hyperref[circ:gqsp]{Fig.~\ref*{circ:gqsp}} alternates $\deg P$ (open-)controlled calls to $W$ or $W^\dagger$ with $\deg P + 1$ single-qubit rotations on the QSP ancilla $\ket{0}_\mathrm{QSP}$: starting with an initial rotation $R(\theta_0,\phi_0,\lambda_0)$ (with $\lambda_0\in\mathbb{R}$ the global-phase parameter), followed by $\deg P$ many rotations $R(\theta_j,\phi_j,0)$ for $j = 1, \dots, \deg P$. This procedure characterizes the top left block of the post-selected circuit on each qubitization subspace as \cite[Thm. 6]{motlagh2024generalized}
...
\end{equation}
where $k$ is the number of controlled-$W^\dagger$ slots (and $\deg P - k$ is the number of controlled-$W$ slots), i.e. any Laurent polynomial $P' \in \mathbb{C}[z,z^{-1}]$ whose ordinary factor $P$ has unit sup-norm on $\mathbb{T}$ is accessible. The rotation angles $\{(\theta_0,\phi_0,\lambda_0)\}\cup\{(\theta_j,\phi_j)\}_{j=1}^{\deg P}$ depend on $P$ only. The slot assignment encodes the negative exponents $z^{-k}$ present in the polynomial. 

The Hamiltonian simulation target $\ee^{-\ii\delta\alpha\cos\theta}$ is a Laurent function of $\ee^{\ii\theta}$, and it is not expressible as a non-trivial ordinary polynomial. (An ordinary $P\in\mathbb{C}[z]$ satisfying $P(\ee^{\ii\theta}) = P(\ee^{-\ii\theta})$ on $\mathbb{T}$ has $P(z) = P(1/z)$ on $\mathbb{T}$, and matching Fourier coefficients forces $P$ to be constant.) Thus, \cite[Thm. 6]{motlagh2024generalized} is exactly what is needed. The bilateral Jacobi-Anger expansion gives, 
...
\end{equation}
Followed up by a truncation at $|k|\leq d < \infty$ yields the Laurent polynomial
$$
L_d(\ee^{\ii\theta}) \;:=\; \sum_{k=-d}^{d} (-\ii)^k\,J_k(\delta\alpha)\,\ee^{\ii k\theta} \;\in\; \mathbb{C}[z,z^{-1}],
$$
of bilateral degree $d$, where the $k$-th Bessel functions $J_k$ have a tail bound 
$$
...
$$
justifying the use of a truncation. To realise $L_d$, introduce the underlying polynomial, 
$$
...
\end{equation}
One GQSP layer then makes $d$ controlled-$W$ calls, $d$ controlled-$W^\dagger$ calls, and $2d + 1$ single-qubit rotations on the QSP ancilla. In the regime $\delta\alpha \lesssim 1$, even $d=1$ suffices:
$$
L_1(\ee^{\ii\theta}) \;=\; J_0(\delta\alpha) \,-\, 2\ii\,J_1(\delta\alpha)\,\cos\theta,
$$
realised by one controlled-$W$, one controlled-$W^\dagger$, and three rotations $(R_0, R_1, R_2)$. Inserting $J_0(z) = 1 + \mathcal{O}(z^2)$ and $2J_1(z) = z + \mathcal{O}(z^3)$ yields
$$
...
|P(z)|^2 + |Q(z)|^2 \;=\; 1.
$$
The existence of such a polynomial is proven by \cite[Thm. 4]{motlagh2024generalized}. But we deviate from their proposed way, and use the contour-integral / FFT construction of \cite{berntson2025complementary}. Since they do not use optimization to solve the problem, they can evaluate $Q$ with explicit error bounds up front, in classical time $\tilde{O}(d)$. Once we have $Q$, using the recursive extraction of \cite[Alg. 1]{motlagh2024generalized} results in the $2d + 1$ rotation triplets from $(P, Q)$ in $\mathcal{O}(d^2)$ arithmetic operations. Both stages are performed once, before any quantum computation, for each value $\delta \alpha$. The same angles are then reused at every quantum call of the layer. 

\smallskip
\subsection{Weak measurement.} \label{sec:prelim-weak-meas}
...
\begin{enumerate}[(i)]
    \item \emph{Block encoding.} $U_L$ acts on $(b, S)$ s.t. on the $\ket{0^b}$ block of the state imprints the amplitude $L/\alpha$ on $S$, leaving an orthogonal artefact on $(b, S)$:
    $$
