...
### qf-e4z.28 single-pass on X+ZZ: structural fix decorative; keep two-pass (2026-05-16)
- See [Single-pass X+ZZ decorative qf-e4z.28](single_pass_xz_disorder_decorative_qf_e4z_28.md). Tested whether the qf-e4z.27 X+ZZ fixtures structurally eliminate the parity-trap inside `predict_lindbladian_trajectory` such that Pass 2 could be dropped. **They do NOT.** Spot cells n=5, n=7 β_phys=2.5 seed=42: single-pass Arnoldi from `vec(I/d)` gives the SAME wrong gap on X+ZZ as Z+ZZ (rel_err 16.6% / 24.3% respectively) — the 0.1 X-disorder is below the MGS orthogonalisation threshold at krylovdim=40. Test E confirms: 1e-10 GUE-perturbed seed (the same seed `_krylov_default_x0` uses for Pass 2) FAILS identically in plain Arnoldi on both fixtures — KrylovKit thick-restart in Pass 2 is what re-amplifies rare parity-odd content via implicit restart; plain MGS cannot. |+⟩⟨+|^⊗N initial state works at n=5 (rel_err 2.5e-7) but fails at n=7 (rel_err 2.6e-5, above the 1e-6 threshold). **Conclusion**: keep [[predict-trajectory-two-pass-qf-e4z-27]] design. No `src/` change. Driver: `scripts/scratch_qf_e4z_28_single_pass.jl`. Full log: `scripts/output/qf_e4z_28_single_pass_log.txt`.
