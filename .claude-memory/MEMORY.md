...
### qf-79h: d_{1→1} normalisation is a no-op for CKG vs DLL (2026-05-25)
- See [qf-79h d11 ratio no-op for CKG vs DLL](qf_79h_d11_ratio_no_op_for_ckg_vs_dll.md). User asked whether the d_{1→1} norm proxy used to "normalise" Lindbladian rates over-corrects for system size, and whether the same-cell ratio R(n,β) = (Gap_CKG/d11_CKG)/(Gap_DLL/d11_DLL) extracts structural differences over pure rate. Empirical answer over qf-e4z.34 × qf-e4z.35: d11_CKG/d11_DLL ∈ [1.0011, 1.0249] (mean 1.013, std 0.007) across all 18 (n,β) cells — d11 grows ~4× from n=3 to n=8 but by the **same** factor for both samplers. Hence R(n,β) ≈ Gap_CKG/Gap_DLL within 2-3%. CKG and DLL share A_i and only differ in α; both α functions have the same KMS magnitude scaling so opnorm(M) cancels almost exactly. **Recommendation: report raw gap and τ_mix ratios as the headline structural metric. d11 normalisation is not needed to defend the structural-vs-rate distinction.** Headline scaling (R=gap_rat at β_phys=0.5): 1.19→1.43→1.41→1.54→1.62→1.70 across n=3..8 — clean structural advantage that grows with n. β_phys=1.0 is parity-modulated (≈0.95 odd-n, ≈1.26 even-n) per [[heisenberg-1d-even-odd-mechanism]]. σ-rate test (within CKG): gap/d11 varies ~50% across σ_factor ∈ [0.25, 2.0] — σ is not a pure rate knob (it sets kink-window structure), so d11 cancels only the rate part; immaterial to the CKG-vs-DLL ratio at fixed σ. Cross-norm robust (HS tracks d11 within 2%). Driver: `scripts/analyze_qf_79h_d11_normalized_ratio.jl`; figures at `drafts/figures/numerics/qf_79h_normalized_ratio_{main,checks}.{png,pdf}`.

### qf-c9g: 2D TFIM ORDERED gap closing is GENUINE ordered-phase physics, NOT a GS-doublet artefact (2026-05-25)
- See [qf-c9g 2D TFIM ordered phase verified](qf_c9g_2d_tfim_ordered_phase_verified.md). 15-cell n × β grid (n ∈ {4,6,8} × β_phys ∈ {0.1, 0.25, 0.5, 1.0, 2.0}) + n=4 dense L slow-mode at all β. Four-of-four diagnostic tests fire: (i) β-persistence at β=0.5 (T/T_c=0.97, eff_rank ∈ [3.3, 8.6] — NOT doublet) with closing 0.449 → 0.286; (ii) Binder U_4 ∈ [0.60, 0.66] saturated; (iii) R_2 lives 89% in doublet × bulk at every β (qf-biz Follow-up A generalises across β — only γ_KMS rate changes, structure constant); (iv) paramagnetic β ∈ {0.1, 0.25} gap flat (slopes ~-0.002, -0.023). Ladder geometry caps closing to floor ~exp(-β·4J) (NOT square-lattice √n) — at β=2: floor 3.4e-4, observed 1.57e-4 ✓. Lit anchor: Gamarnik-Kiani-Zlokapa 2024 (arXiv:2411.04300) Thm 1.4 proves T_mix ≥ exp[n^(1/2-o(1))] for our exact CKG KMS-DB sampler class. All qf-1jj/qf-biz baselines reproduced bit-accurately.

### qf-e4z.41/.42: n=8 at r_D=12 is ~10-12 h laptop / ~3-4 h cluster (2026-05-25)
- See [qf-e4z.41 n=8 r_D=12 laptop wall](qf_e4z_41_n8_r_d12_laptop_wall.md). qf-e4z.41 GNS smooth-Metro β_phys=0.5 seed=46 sweep closed partial: n=3 σ-ramp + n=4..7 σ_f=1/16 (9/10 cells, all :extrapolated, all_converged=true). n=8 cell at r_D=12, kdim=100 was empirically ~10-12 h on the 8-core laptop and was aborted; filed as qf-e4z.42 (cluster-grade). Reason: r_D=12 is 32× more ω-labels per matvec than r_D=7, plus L2-cache-miss at d=256 (1 MB per scratch buffer) adds another ~2-4×. Per-cell wall ratio is ~10-12× per +n on the EnergyDomain Lindbladian path. Comparison with qf-e4z.34 CKG at matching cells: GNS @ σ_f=1/16 vs CKG @ σ_f=1 ratios within ~7% on both gap and τ_mix for n≥4 (as expected — both asymptote to the Davies generator as σ→0). Mild even/odd-n parity signal visible at β_phys=0.5 (much weaker than qf-e4z.31 high-β regime).

### qf-e4z.35.1: σ-sweep V-shape is a population↔coherence eigenmode swap (2026-05-25)
- See [qf-e4z.35.1 spectral mode swap](qf_e4z_35_1_spectral_mode_swap.md). Avoided crossing at $c^\star(n,\beta_{\mathrm{phys}})$ — below $c^\star$ slow L mode is population (ρ_0 has $|c_2|^2 \sim O(1)$); above, coherence ($|c_2|^2 \sim 10^{-5}$). Sharp jump, gap unchanged. Intra-sampler analogue of [[qf-e4z-34-ckg-vs-dll-plot-sweep]].

### qf-0fv: Pass-2 spectral_gap gated behind `compute_true_gap=false` kwarg (2026-05-25)
- See [qf-0fv Pass-2 gated](qf_0fv_pass2_gated.md). `predict_lindbladian_trajectory` / `predict_channel_trajectory` now skip the qf-e4z.27 `krylov_spectral_gap` Pass-2 call by default. `spectral_gap` field is sourced from Pass-1: `|Re(λ_2)|` for L, `-log|μ_2|/δ` for channel (matches qf-e4z.37 driver convention). Opt in via `compute_true_gap=true` only for parity-symmetric `(rho_0, L)` audits (e.g. `ρ_0 = I/d` on clean H) — not for production sweeps. Reduces matvec cost ~50 %. Two regression tests opt in explicitly. Code at `src/lindblad_action.jl:662, 868`.
...
- **Confirmed project-wide, not model-specific (qf-b4i.1, 2026-05-25)**: tested |0⟩⟨0|^⊗N as an X-parity-breaking alternative on 2D TFIM (where the project-canonical |+⟩ is X-parity-EVEN). Hypothesis ("|0⟩ should track 1/λ_2 in ORDERED") was refuted: |c_2|(|0⟩) ≤ 10⁻¹¹ at every cell because |0⟩⟨0|^⊗N lives in the doublet × doublet block while L's R_2 lives in the doublet × bulk block. Operator-level parity-breaking is necessary but not sufficient; the eigenbasis block structure of ρ_0 is what matters. See [[qf-b4i-1-2d-tfim-x-parity-rho0-refuted]].
...
### Minimal-simulation scope feedback (2026-05-24)
- See [feedback: minimal simulation scope](feedback_minimal_simulation_scope.md). When the user asks for a small physics diagnostic, default to the MINIMUM simulation that answers the specific question — do not inherit knobs (multi-kdim, multi-seed, Bohr cross-check, trajectory τ_mix) from existing sweep harnesses by reflex. Read the relevant driver, list its knobs, and trim each one unless the question demands it.

### qf-b4i.1: 2D TFIM |0⟩⟨0|^⊗N X-parity-breaking ρ_0 refuted; canonical |+⟩ stays (2026-05-25)
- See [qf-b4i.1 X-parity ρ_0 refuted](qf_b4i_1_2d_tfim_x_parity_rho0_refuted.md). Tested |0⟩⟨0|^⊗N (X|0⟩=|1⟩ ⇒ X-parity-MIXED density operator under P=X^⊗N, hypothesised to expose L's slow Z₂-tunnelling mode) on the qf-b4i 6 cells. Audit ✓: gap_phys agrees with qf-65e Pass-2 to ≤4e-10 at every cell — Pass-1 from |0⟩ captures L's slowest eigenvalue. **Hypothesis refuted**: |c_2|(|0⟩) ≤ 10⁻¹¹ at every cell, τ_mix(|0⟩)^ORD constant at ∼17 alg across n∈{4,6,8} — shorter than both |+⟩ (251.8 at n=8) and I/d (190 at n=8). Mechanism: in deep-ordered |0...0⟩ ≈ (|ψ_1⟩+|ψ_2⟩)/√2 ⇒ ρ_0 supported on doublet × doublet block of energy eigenbasis, while R_2 lives in doublet × bulk block (qf-biz Follow-up A) — orthogonal. Operator-level X-parity breaking is necessary but NOT sufficient for coupling to R_2; what matters is the eigenbasis block structure of ρ_0. **NO model-specific ρ_0 switch warranted; project-canonical |+⟩⟨+|^⊗N stays**. Driver: `scripts/scratch_qf_b4i_1_2d_tfim_taumix_zero.jl` (bypasses Pass-2 via direct `_krylov_spectral_decomposition` call; wall 14m on 8 Julia threads). Sidecars: `_rho_zero.bson` family. Plot regenerated with three audit columns at `drafts/figures/numerics/qf_biz_taumix_and_gap_2d_tfim.{png,pdf}`. Drafts updated: 2d-tfim-ordered-vs-disordered.md Headline (4) + Methodology; qf-biz Follow-up D.

### qf-biz: 2D TFIM ORDERED L gap closing is NOT bare within-doublet matrix element bottleneck (2026-05-24)
- See [qf-biz 2D TFIM matrix element refuted](qf_biz_2d_tfim_matrix_element_refuted.md). The "matrix-element bottleneck" story I floated for the qf-1jj 2D TFIM ORDERED gap closing (gap_phys = 9.96e-2 → 3.06e-3 → 1.57e-4 across n=4,6,8 at h=J=1, β_phys=2) — proposed in response to user skepticism about a Gibbs-on-GS artefact — IS WRONG. M²_norm = Σ_a|⟨ψ_2|A_a|ψ_1⟩|² is CONSTANT at 0.311 from n=4 to n=6 (sum-rule: σ_z ~ 1/site, summed × 1/(3n) = 1/3). σ_x = 0 by Z₂; σ_y small. The actual slow mode (dense L eigendecomp at n=4) is a Z₂-mixed coherence living 89% in the doublet × bulk off-diagonal block (top entries |ψ_{1,2}⟩⟨ψ_8|-type) — NOT within-doublet — magnetisation-orthogonal. Closing involves the full Davies-rate-equation eigenvalue over doublet-to-bulk coherences (energy-increasing → Boltzmann-suppressed γ). PHASE confirmed at β=2 (eff_rank=2.00, ⟨M_z²⟩=0.945, U_4=0.656, distinct from [[heisenberg-1d-even-odd-mechanism]] pure-GS regime). β-non-monotonicity in λ_L(β) (min at β=1.5) is REAL (kinky-Metro matches smooth shape to 4%), not a filter artefact — slow-mode crossover, c.f. Bando 2018. qf-1jj draft needs a 2-sentence footnote pointing here, no headline edits.

### qf-e4z.40: n=7 β=1.0 outlier — accidental ρ_0/eigenmode near-orthogonality, NOT a code bug (2026-05-22)
- See [qf-e4z.40 n=7 β=1 outlier resolved](qf_e4z_40_n7_b1_outlier_resolved.md). After qf-e4z.37 Pass-1 fix, n=7 β_phys=1.0 seed=46 retained 27% gap_ratio while other 17/18 cells are within 8.5%. Three matrix-free experiments establish: (1) value robust across kdim {60,100,150}, r_D {7,8}, δ {1e-3,5e-4} — refutes Krylov/quadrature/Trotter explanations; (2) L's slowest mode at this cell has |c_2| = 6.7e-5 (~10⁴× smaller than any other cell; ρ_0 = |+⟩⟨+|^⊗N is accidentally near-orthogonal to L's λ_2 eigenvector at this disorder seed × n × β); (3) DECISIVE seed-from-L test: seeding Φ_δ Pass-1 with L's λ_2 eigenvector finds Φ_δ eigenvalue at μ=0.999897 (matches L-predicted exp(-δ·0.10247)=0.999898 to 5e-7). Φ_δ STRUCTURALLY preserves L's slowest mode to O(δ²); the standard ρ_0 seed just doesn't excite it. The reported channel gap_phys=5.52 IS the trajectory-effective gap of |+⟩⟨+|^⊗N (rate at which actually-populated modes decay; matches L's λ_4 = 0.128 cluster within 1e-6). τ_mix inflation only 14% (1.59 vs 1.39 phys), well within qf-e4z.37 ≤20% bound. **No code fix needed**; plot can use Pass-1 number as-is. The "27%" is comparing the slowest operator-eigenvalue of L vs the slowest-rho_0-coupled eigenvalue of Φ_δ — physically meaningful difference, not a numerical error.
