...
### Krylov x_0 must break symmetry; gap_alg ≠ gap_phys (qf-8fr, 2026-05-13)
- See [Krylov x_0 symmetric-system bug qf-8fr](krylov_x0_symmetric_bug_qf_8fr.md). Two findings: (1) pre-patch `krylov_spectral_gap` seeded Arnoldi with `vec(I/d)` — symmetry-protected on clean Hamiltonians, silently returned wrong gap (3.8× overestimate on classical Ising n=4). Patched: `_krylov_default_x0` builds `vec(I/d + 1e-10·H_GUE_traceless)`. Disordered Heisenberg unaffected (~1e-14 rel agreement with dense). Full sandbox suite: 5518/5518. (2) Reported λ_gap is `λ_alg` in rescaled frame H_alg = H_phys/R + s·I, R∝n for extensive H — so a true Ω(1) physical gap shows up as ~1/n in algorithm units. Conversion: λ_phys = λ_alg · ham.rescaling_factor. The "n^{-1.2}" Heisenberg decay on qf-e4z.21 is this artefact: λ_phys is approximately flat across β_phys∈{0.25,0.5,1.0} for both Ising and Heisenberg at n≥4 (slopes in [-0.2, +0.2]). See `drafts/classical-ising-crosscheck.md` for full discussion + theorem citations (KB16/Bardet23/BC25).
