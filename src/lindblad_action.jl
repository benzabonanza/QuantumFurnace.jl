...
# Pipeline A — trajectory / mixing-time (qf-umr)
This is the **state-specific simulation** pipeline. It simulates the actual
relaxation of a GIVEN physical initial state ρ₀ (canonical `|+⟩⟨+|^⊗N`, the
2026-05-24 policy) and produces the trajectory, τ_mix(ε) (via
`eigenmode_mixing_time`), and a FREE ρ₀-coupled gap read off the SAME single
Arnoldi factorisation (`spectral_gap`, the slowest non-steady mode of the
ρ₀-seeded decomposition). This is a simulation of ρ₀, NOT an operator-spectrum
estimate — see `krylov_spectral_gap` (Pipeline B) for the latter.

Per qf-x6j the trajectory and the operator gap are DIFFERENT numerical problems:
powering `e^{tL}ρ₀` only needs the Krylov subspace to span ρ₀'s orbit and is
therefore krylovdim-robust, whereas resolving the true operator gap (a clustered
eigenvalue of a non-normal generator) needs a thick restart + a free seed. The
Pass-1 gap here is the ρ₀-coupled rate, which IS the trajectory's decay rate; on
P̂-symmetric `(ρ₀, L)` pairs it differs from the true operator gap and the opt-in
`compute_true_gap=true` defers to `krylov_spectral_gap`. The Pass-1 gap is a
zero-cost byproduct of the trajectory decomposition — keeping it coupled here
avoids a second decomposition (the one coupling that is genuinely efficient).
...
- `compute_true_gap::Bool = false`: thin convenience wrapper around
  Pipeline B. When `true`, run a second `krylov_spectral_gap` call
  (qf-e4z.27 "Pass 2") seeded with `_krylov_default_x0 = vec(I/d +
  1e-10·H_GUE)` and KrylovKit thick restart, and report its result as
  `spectral_gap`. Use this only when the caller specifically needs the
  true Lindbladian gap on a P̂-symmetric `(rho_0, L)` fixture (e.g.
  `rho_0 = I/d` on a parity-symmetric Hamiltonian) — for such fixtures
  Pass-1 reports the P̂-even sub-spectrum gap, which is what the
  trajectory of `rho_0` actually decays at, but is *not* the true L gap.
  With the canonical `rho_0 = |+⟩⟨+|^⊗N` policy + adequate krylovdim
  (qf-e4z.33), Pass-1 matches the true gap to machine precision, so the
  default is `false`. qf-umr: the already-built Workspace is forwarded
  into the Pass-2 call (`workspace=ws`) so the O(d^3.x) operator build is
  not paid twice; the Pass-2 Arnoldi still runs separately, so
  `total_matvecs` grows but the build does not.
...
function predict_lindbladian_trajectory(
    ...
    if compute_true_gap
        ...
        # qf-umr: forward the Workspace we already built into the Pass-2 gap
        # call so the O(d^3.x) operator build is not paid a second time. `ws`
        # was built (or reused) above for exactly this (config, ham, jumps), so
        # krylov_spectral_gap's reuse guard accepts it and the result is
        # bit-identical to a self-built workspace. The Pass-2 Arnoldi still runs
        # its own factorisation (free seed, :LR targeting) — only the build is
        # shared.
...
# Pipeline A — trajectory / mixing-time (qf-umr)
The channel counterpart of `predict_lindbladian_trajectory`: simulate the actual
relaxation of a GIVEN ρ₀ under the implemented Φ_δ and produce the trajectory,
τ_mix(ε), and a FREE ρ₀-coupled gap (`spectral_gap = |log|μ_2||/δ`, Pass-1) off
the SAME single Arnoldi factorisation. Per qf-x6j this is a different numerical
problem from the operator-spectrum estimate: powering Φ_δ^k ρ₀ is krylovdim-robust
(qf-x6j proved the channel τ_mix is krylovdim-robust), whereas the operator gap
of the non-normal Φ_δ is fragile (the Pass-2 `krylov_spectral_gap` channel route
can pick the wrong mode at stiff cells, qf-e4z.37 — and qf-9lp shows the robust
channel operator gap actually comes from 𝓛's λ₂, not Arnoldi on Φ_δ). See
`krylov_spectral_gap` (Pipeline B). The Pass-1 gap stays coupled to the
trajectory because it is a zero-cost read off the same decomposition.
...
- `compute_true_gap::Bool = false`: thin convenience wrapper around
  Pipeline B. When `true`, run a second `krylov_spectral_gap` call
  (qf-e4z.27 "Pass 2") and report its result as `spectral_gap`. **Off by
  default** because Pass-2 on the channel path is known to pick the wrong
  eigenmode at stiff Trotter cells (qf-e4z.37: 11-47% inflation,
  krylovdim-independent; qf-9lp: the robust channel operator gap is 𝓛's
  λ₂, not Arnoldi on Φ_δ). qf-umr: the already-built Workspace is
  forwarded into the Pass-2 call (`workspace=ws`) so the operator build is
  not paid twice; the Pass-2 Arnoldi still runs separately. The Pass-1
  default reports `|log|μ_2||/δ` from the rho_0-aligned Arnoldi
...
function predict_channel_trajectory(
    ...
    if compute_true_gap
        kdim_gp = krylovdim_gap_pass === nothing ? max(30, Int(krylovdim)÷2) : Int(krylovdim_gap_pass)
        # qf-umr: forward the Workspace we already built into the Pass-2 gap
        # call so the operator build is not paid a second time. `ws` was built
        # (or reused) above for exactly this (config, ham, jumps; trotter), so
        # krylov_spectral_gap's reuse guard accepts it and the result is
        # bit-identical to a self-built workspace. The Pass-2 Arnoldi still runs
        # its own factorisation (free seed, :LM targeting) — only the build is
        # shared. (Pass-2 on the channel can pick the wrong eigenmode at stiff
        # Trotter cells, qf-e4z.37 — hence opt-in only; reuse changes cost, not
        # the number.)
        gap_res = krylov_spectral_gap(
            config, hamiltonian, jumps;
            trotter=trotter,
...
- `n_values`: vector of qubit counts. Each `n` requires
  `<hamiltonian_dir>/heis_xxx_disordered_periodic_n<n>_seed46.bson` to exist.
...
function sweep_channel_mixing(
    ...
    for i in 1:n_points
        ...
        try
            ...
            # returns the qf-e4z.20 TrotterTriple for KMS coherent (three
            # independent per-leg Strang caches).
