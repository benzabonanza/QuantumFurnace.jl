#!/usr/bin/env julia
#
# qf-fqt — TimeDomain dissipator quadrature at β_phys=2, n=5, smooth_def_s.
#
# Companion to scratch_quad_S1_bphys2_krylov.jl (which measured EnergyDomain).
# Here the TEST side is TimeDomain (FINUFFT-driven), reference is still
# EnergyDomain at r_D = R_REF (= 12 by default). The recipe v2 established
# that L_E(r=12) ≡ L_B at machine matrix-element precision, so this
# directly measures
#
#   ‖ L_T(r_test)  -  L_B ‖_op
#
# with no BohrDomain dense build (which is infeasible at n=6 and 256 MB at
# n=5). All matvecs go through `apply_lindbladian!` matrix-free; OMEGA-loop
# threading is on (JULIA_NUM_THREADS, BLAS=1).
#
# Why n=5: the user is worried the TimeDomain path blows up. At n=5, β_phys=2
# we have β_alg ≈ 88, s = default_smooth_s ≈ 19.6 — the same regime as the
# EnergyDomain run that needed r_D = 7, but TimeDomain adds a FINUFFT
# precision floor (~1e-12) and a different per-matvec scaling. n=5 is
# diagnostic: large enough that the matvec cost is non-trivial, small
# enough that we can push r_test up to 12 if needed.
#
# Run with:
#   JULIA_NUM_THREADS=8 julia --project scripts/scratch_quad_S2_bphys2_time_krylov.jl
#
# Env overrides:
#   N_LIST        comma-separated n list   (default 5)
#   BETA_PHYS     β_phys                   (default 2.0)
#   R_MIN, R_MAX  test r_D sweep range     (default 3..12)
#   R_REF         reference r_D            (default 12)
...
# ── Campaign grid ───────────────────────────────────────────────────────────
const N_LIST    = parse.(Int, split(get(ENV, "N_LIST",   "5"), ','))
...
const R_MAX     = parse(Int, get(ENV, "R_MAX", "12"))
const R_REF     = parse(Int, get(ENV, "R_REF", "12"))
const TAIL_C    = parse(Float64, get(ENV, "TAIL_C", "8.0"))
const ETA       = parse(Float64, get(ENV, "ETA", "1e-3"))
const CELL_TIME_BUDGET = parse(Float64, get(ENV, "CELL_TIME_BUDGET", "3600.0"))
...
const OUTDIR      = get(ENV, "OUTDIR",
                        joinpath(@__DIR__, "output", "quad_redo_S2_bphys2_time_krylov"))
...
# ── Per-cell config builder — supports both Energy and Time test sides ──────
# TimeDomain `_precompute_data` for KMS construction always builds the
# b_minus / b_plus time grids even when the matvec runs with
# `include_coherent = false`, so we have to pass valid kwargs for those
# registers. Setting them equal to the D register keeps the precompute
# cheap and the values are unused downstream.
function build_cfg(domain, n_qubits, beta_alg, sigma, omega_range,
                   filter::Symbol, r_D::Int, beta_phys::Float64)
    ...
    common = (
        ...
        num_energy_bits_b_minus = r_D, w0_b_minus = w0_D, t0_b_minus = t0_D,
        num_energy_bits_b_plus  = r_D, w0_b_plus  = w0_D, t0_b_plus  = t0_D,
    )
    if filter === :gaussian
        sigma_gamma = sigma
...
# ── Mixed-domain matrix-free op-norm (test=TimeDomain, ref=EnergyDomain) ────
function op_norm_matrix_free_mixed(ws_test, cfg_test, ws_ref, cfg_ref, ham, d::Int;
...
function run_cell(n_qubits::Int, beta_phys::Float64, filter::Symbol)
    ...
    cfg_ref = build_cfg(EnergyDomain(), n_qubits, beta_alg, sigma, omega_range,
                        filter, R_REF, beta_phys)
    ...
    for r in R_MIN:R_MAX
        ...
        cfg_test = build_cfg(TimeDomain(), n_qubits, beta_alg, sigma, omega_range,
                              filter, r, beta_phys)
        w0_D = omega_range / 2^r
        t_build_ws_test = @elapsed ws_test = Workspace(cfg_test, ham, jumps)
        t_norm = @elapsed (op_norm, converged) = op_norm_matrix_free_mixed(
    ...
    cutoffs = Dict(
        ...
        :eps_1e_12 => let i = findfirst(row -> row.err <= 1e-12, rows); i === nothing ? missing : rows[i].r end,
    )
    ...
    BSON.bson(sidecar_path, Dict(
        :cell => (
            ...
            r_ref = R_REF, test_domain = :TimeDomain, ref_domain = :EnergyDomain,
            max_iter = MAX_PI_ITER, rel_tol = REL_TOL,
            min_iter = MIN_PI_ITER, abs_floor = ABS_FLOOR,
...
println("\n=== Campaign summary ===")
@printf "%-3s %-6s %-18s %10s %8s %8s %8s %9s\n" "n" "β_phys" "filter" "wall_s" "r@1e-3" "r@1e-6" "r@1e-9" "r@1e-12"
for r in campaign_summary
    @printf "%-3d %-6.2f %-18s %10.1f %8s %8s %8s %9s\n" r.n_qubits r.beta_phys string(r.filter) r.cell_wall (
        ...
        ismissing(r.cutoffs[:eps_1e_12]) ? "—" : string(r.cutoffs[:eps_1e_12])
    )
end
