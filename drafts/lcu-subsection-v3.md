# Linear Combination of Unitaries (v3)
...
- **E5 (composite $\alpha$ display label).** Deliberately *not* labelled in v2/v3; the display sits inside paragraph 5's argument flow and does not need to be cross-referenced by later chapters (Chapter 5 already has the same formula at `2_methods.tex:1726`, and the Numerics chapter can cite that). The review flagged this as optional.
...
**Single surgical change: the forward pointer to §sec:prelim-QSP was retargeted to Motlagh–Wiebe GQSP applied to the qubitization walk $W = R_b U_B$, reflecting that the QSP subsection actually uses Motlagh–Wiebe GQSP (not Low–Chuang QSP).**

The v2 draft already named GQSP in paragraphs 5 and 6 and already cited `motlagh2024generalized` at the degree-bound display. v3 tightens the attribution in three places:
...
- **Paragraph 5, the "absorbed at the next circuit layer" sentence.** Rewritten so that the walk operator $W = R_b U_B$ appears **inside** the sentence that names Motlagh–Wiebe GQSP (not only in paragraph 6), and [Motlagh and Wiebe 2024] is cited by name (not only as `motlagh2024generalized` in the degree formula). The Laurent-polynomial target $P(\ee^{\ii\theta}) \approx \ee^{-\ii\delta\alpha\sin\theta}$ from `2_methods.tex:1731` is now explicitly stated here (v2 only had it implicit via the degree formula).
