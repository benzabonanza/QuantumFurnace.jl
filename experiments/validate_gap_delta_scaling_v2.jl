#!/usr/bin/env julia
# ============================================================================
# Delta-Scaling Revalidation (v2: longer mixing, uniform psi0)
# ============================================================================
#
...
#
# Changes from v1 (validate_gap_delta_scaling.jl):
...
#   - Quick-30: Gap error NOT O(delta) -- error/delta varies 96x. Richardson
#     extrapolation ineffective (1.0x). Dominant error is systematic observable
#     bias, not Trotter discretization.
#
# Hypothesis: Previous script used mixing_time~5/gap (~5) and excited-state
# psi0 -- may not be in the correct asymptotic regime for exponential gap
# convergence. Longer mixing (20) + uniform superposition should ensure all
# Hilbert space subspaces are covered and the system has time to equilibrate.
#
...
#
# Usage:
#   cd QuantumFurnace.jl && julia -t 4 --project experiments/validate_gap_delta_scaling_v2.jl
# ============================================================================
...
println("=" ^ 70)
println("  TROTTER DELTA-SCALING REVALIDATION (v2: longer mixing, uniform psi0)")
println("  beta=$BETA, ntraj=$NTRAJ, seed=$SEED")
println("  Deltas: $DELTAS")
println("  mixing_time=20 (fixed), skip_initial=0.3, psi0=uniform superposition")
...
# Initial state: uniform superposition (all subspaces covered)
psi0 = ones(ComplexF64, dim) / sqrt(dim)

# Mixing time: fixed at 20 (longer than v1's ~5/gap)
mixing_time = 20.0
...
for delta_val in DELTAS
    ...
    gap_result = estimate_spectral_gap(jumps, config_t, psi0, ham;
        ...
        skip_initial=0.3, delta=delta_val)
...
@printf("v2 changes: mixing_time=20 (fixed), skip_initial=0.3, psi0=uniform superposition\n\n")
